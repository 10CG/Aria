...
**⭐ 分族规则 (U-5 裁定走结构分, 成文以防下一轮读者重算出不同的数)**: 取 issue 引用**渲染后的可见文本** (剥 markdown 链接语法、保留链接文字), 按**最后一个** `#` 拆出 `left`; `left` 为空 ⇒ **D**; `left` 含 `/` ⇒ 有空格 **C** / 无空格 **B**; `left` 不含 `/` ⇒ 有空格 **F** / 无空格 **A**。**纯结构判定, 不引入「这个 token 是不是真仓名」的语义知识** —— 因为解析器没有那条知识, 用它分族就是拿实现里不存在的区分去论证实现的覆盖面。

| 族 | 形态 | 已落盘总体 (16) | 复制源总体 (11) |
|---|---|---|---|
| A | `aria-plugin#122` (裸 token, 无空格) | **4** | **1** |
| B | `10CG/aria-plugin#110` (org 限定, 无空格) | **11** | **0** |
| C | `10CG/aria-plugin #122` (org 限定 + 空格) | **0** | **4** |
| D | `#137` (**无 repo 段**) | **0** | **1** |
| **F** ⭐ | `aria-plugin #95` (**裸 token + 空格**, token 在链接文字之外) | **0** | **5** |
| E | 逐字照抄整个字段值 (markdown 链接外壳) | **0** | **11** |

*(已落盘总体另有哨兵 1 条 `AUDIT-TEST-DO-NOT-USE#0`, 不入族; 4+11+1 = 16。复制源 1+0+4+1+5 = 11。)*

> **⭐ F 族的两个数字, 两个用途 (U-5 第三条路的要点 —— 这张表被两个要不同数的论证同时使用)**:
> - **形状频次 = 5** —— SC-1 第四元要覆盖的**结构**在复制源总体出现 5 次。这是**解析器视角**, 机械可复现。
> - **其中真实仓名 = 2** —— 5 个 token 里 `aria-plugin` ×2 是真仓, `Forgejo` ×3 是平台名 (不是仓)。**只有这 2 例能真正造成跨格式碰撞**; 另 3 例归一后得 `forgejo#N`, 与任何真实仓的键都不相等 ⇒ 结构上走 F 路径, 后果上无害。
> - ⇒ **论证 SC-1 覆盖面用 5, 论证碰撞风险用 2。两个数各自标口径, 不互相冒充。**
>
> **复现命令**: `git ls-files 'openspec/**/proposal.md' | xargs grep -h '^\s*>\?\s*\*\*关联 Issue\*\*'` → 12 行; 其中 **5** 行尾部带 `#issuecomment-NNNNN` 锚点 (`| grep -c '#issuecomment-'`)。

> **口径 (四列不可相加)**: A/B/C/D/F 按渲染可见文本分族, **互斥**, 复制源合计 **11**; **E 族是外壳轴, 与前五族正交** —— 复制源 **11/11 全覆盖** (逐字照抄整个字段值得到的一定是 E 族, 形如 `Forgejo [#137](https://…/issues/137#issuecomment-12236)`), 按规则 1 取**最后一个** `#` 会切到 `issuecomment-12236))` 这类串 ⇒ **E 族一律不可解析**, 退回步骤 4 原串比较。
