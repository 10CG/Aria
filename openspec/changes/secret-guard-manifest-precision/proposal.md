...
- **适用面点名 (R1 A2-M1/A3-M1)**: 全部**路径清单型** pattern 行 — `:709` shell-rc 行、本 spec 新增 claude-config 行、以及 `.env`/`id_rsa` 等同构 sibling 行 (B.1 入场时对 pattern 数组逐行枚举并在 detailed-tasks 列清单; 活体误拦命中的正是 .env 面, 只改 shell-rc 行治不到)。命令注入型 pattern (base64/sops 等) 不适用。**⚠️ 本 bullet 的白名单集合与适用面已由文末 Amendment-1 修正 (两族白名单 + 枚举适用集), 以 Amendment-1 为准。**
...
## Amendments (append-only, Phase B 实现期; mid_post_spec checkpoint 配置 off ⇒ 无审计轮, 由主 loop 记录并请 owner 复议)

### Amendment-1 (2026-08-22, TASK-007 枚举期) — What.3 白名单按「全名型 / 后缀型」两族定义; 适用行按 FP 形状精确枚举

- **原陈述** (What.3): 单一白名单 {串首, 空白, `"`, `'`, `=`, `/`}, 适用「全部路径清单型 pattern 行」。
- **实现期证据**: 对后缀型敏感名 (`\.env` / `\.pem` / `id_rsa` …) 字面套该白名单会放走 `cat prod.env`、`find -name '*.env'` 等今天被拦的合法形态 (前缀 `d` / `*` 不在集内) ⇒ 漏报回归; SC-5 守卫只列 shell-rc 全名形态, 对此盲。
