...
ALTER TABLE `dispatches` 加 5 column:
| col | type | 用途 | 来源 |
|---|---|---|---|
...
- 同 silknode-integration-contract 契约 1 no-storage 透传
...
- fallback_chain_json dict 写入 (write-time, schema v2)
...
- Hermes restart 后, cron tick 进入 `_handle_s5_await`:
  ...
  - 状态分支: pending/queued → leave (next tick) / running/complete → S6_REVIEW / failed/lost → S_FAIL
...
- 时机: T13 mid Phase B.2 (per OD-12 §Q8d)
...
- compute_cost provider-aware branch: Luxeno=0 (flat sub), Zhipu metered
...
| AD-M3-8 | _spillover_ | _待回填_ | _on demand_ |
| AD-M3-9 | _spillover_ | _待回填_ | _on demand_ |
| AD-M3-10 | _spillover_ | _待回填_ | _on demand_ |

> **AD slot backfill 强制 closeout** (per `feedback_ad_slot_backfill_checkpoint`): T16 closeout 必须 fail-fast if 任何 AD 仍 `_待回填_`; 用 sentinel grep 在 m3-handoff validator 内强制。
...
Phase B.2.1       M3 新 scope (T5-T11, ~90h):
...
## Owner Action Items (4 个, per OD-12 §Owner action items)
...
3. **Phase B.2.Z T13**: 执行 secret rotation (5 keys, ~45min per SOP `.aria/decisions/2026-05-02-secret-rotation-deferred.md`)
