...
> **Status**: ✅ **B.2 实施完成 2026-09-02** (22/25 勾选; 5.3 / 5.4 本地合并 + tag + gitlink 已做, **双推待 owner 授权**; 5.6 主仓 PR 待推送授权) — aria `fe32441` (v1.68.0, tag 已建) / standards `fad8b4b` / 主仓 feature/linked-issue-field-availability; 测试 48/48 + state-scanner 1457 + run_all_tests 1889 全绿; Rule #6 AB 见 `aria-plugin-benchmarks/ab-results/2026-09-02-v1.68.0-linked-issue-field-rule6/RESULT.md` (无 WITHOUT_BETTER); B 期技术裁定 B1–B7 见决策单 2026-09-01 追记。历史: ✅ **A.2 + A.3 complete** (2026-08-30 owner 批准进 A.2; post_planning R1 FAIL → R2 PwW → R3 PwW → **R4 CONVERGED 2026-08-31 (五席 5/5 PASS, 0C 0M)**; 收敛后定点 minor 编辑见文末「R4 收敛后定点编辑」) — 全部任务 `pending`, ready for B.1 (待 owner: 版本档 / O-1 / O-3)
...
- [ ] 5.3 aria 子模块 **本地** merge feature 分支 → master + `git push origin && git push github` + **逐个** `git ls-remote <remote> master` 与本地 SHA 比对全部一致 + tag `v<ship>` 双推 + 主仓 gitlink bump 到 post-merge master SHA (硬约束 1/2; push 显式给足超时, memory `partial-push`) (2026-09-02 已做: 本地 --no-ff merge fe32441 + tag v1.68.0 + 主仓 gitlink; **双推 + 逐 remote ls-remote 待 owner 授权**)
- [ ] 5.4 standards 子模块 **本地** merge → master + 双推 + 逐 remote `ls-remote` 核验 + 主仓 gitlink bump (**跨仓交付面**; aria-standards 实测无 `VERSION` / `CHANGELOG.md` / tag, 只做 commit + 双推 + gitlink; 是否版本化留 owner) (2026-09-02 已做: 本地 --no-ff merge fad8b4b + 主仓 gitlink 同 commit; **双推待 owner 授权**; 不版本化, 决策单 B4)
