...
> **Status**: Draft
> **Effort baseline**: ~33h impl (TG-DOCS-A ~11h + TG-DOCS-B ~22h). Single SoT — matches proposal.md §Effort baseline and frontmatter.
...
> **AD allocation**: AD-M5-11 (claimed, T-B6), AD-M6-7 (probes design, T-A6), AD-M6-8 (reserved, T-B6)
...
- [ ] T-A1.3 Apply Diff 3: add §两层 AI 分工 subsection after §十步循环. Max 20 lines. Cross-link AD1 + AD6.
...
```bash
# Rule #1-#6 text must be identical to pre-edit. Use git diff to verify.
# If any line in Rules #1-#6 block appears in the diff as a deletion, STOP.
...
**Subtasks**:
- [ ] T-A2.1 Update Plugin version badge: change `v1.15.2` → actual version from `aria/.claude-plugin/plugin.json` (current: v1.27.0 per commit `c7e611f`). Verify badge URL still resolves by inspecting badge format.
...
**Subtasks**:
- [ ] T-A6.1 Write Probe 1 `m6-version-badge-match` per proposal.md §A.6. Test PASS case: ensure README badge and plugin.json version match (should pass after T-A2.1). Test FAIL case: temporarily edit plugin.json version to a dummy value, run probe, verify exit 1, restore.
...
- [ ] T-A6.3 Write Probe 3 `m6-arch-doc-stale` per proposal.md §A.6. Test PASS case: run after T-B1 (system-architecture.md Last Updated = 2026-05-24), verify exit 0. Test FAIL case: temporarily set Last Updated to 2025-01-01, verify exit 1 with STALE message.
...
**P-11 content boundary**: `standards/autonomous/` files = curated Lab-shareable patterns + rubric guide. `aria-orch/evals/m6-prompt-quality/corpus/` = raw M6 E2E corpus (Spec #2 TG-C). Cross-reference only; no content duplication.
...
- [ ] T-B3.2.2 Add Lab-shareable header: `<!-- Lab-shareable: this file is in standards/ and may be reused by other 10CG Lab projects. For M6 E2E corpus samples, see aria-orch/evals/m6-prompt-quality/corpus/ (Spec #2 TG-C). -->`.
- [ ] T-B3.2.3 Write §PRD §639 Rubric section: scoring dimensions (naturalness / actionability / context-completeness / brevity / tone-appropriateness). Explain the ≥7/10 median threshold.
...
- [ ] T-B3.2.5 Write §BOTH-Locations Design Note: "This file contains Lab-shareable curated patterns. The M6 E2E run corpus samples (raw data with per-sample scoring) are in `aria-orch/evals/m6-prompt-quality/corpus/sample-{01..10}.md` (Spec #2 TG-C). These two locations serve different purposes and do not duplicate content."
- [ ] T-B3.2.6 Add cross-reference: "See also: `aria-orch/evals/m6-prompt-quality/` for M6 E2E corpus samples and scores (Spec #2 TG-C deliverable)."
...
**Description**: Create `/home/dev/Aria/aria-orchestrator/docs/layer-boundary-contract.md`. This file is Aria-specific and must NOT be placed in standards/ (per km M-km-R2-005 decision documented in AD-M5-11).

**Subtasks**:
- [ ] T-B4.1 Create file with Aria-specific header: `<!-- Aria-specific: this file is NOT Lab-shareable. It belongs in aria-orchestrator/docs/, not in standards/. Per km M-km-R2-005 decision recorded in AD-M5-11. -->`.
...
### T-B6: architecture-decisions.md AD-M5-11 claim + AD-M6-7/8 stubs (~0.5h)
...
**Subtasks**:
- [ ] T-B6.1 Locate the AD-M5-11 RESERVED section (line ~3460 per grep output). Replace the RESERVED status with the Decided content: document the `standards/autonomous/` namespace creation decision. Content:
...
- [ ] T-B6.2 Add AD-M6-7 entry documenting the state-checks probe design decision:
  ...
  - **决策**: 3 probes in .aria/state-checks.yaml: version-badge-match / claude-md-version / arch-doc-stale. Severity = warning (non-blocking). POSIX tools only (grep, date, python3). `enabled: true` even pre-v2.0 so state-scanner surfaces status proactively.
- [ ] T-B6.3 Add AD-M6-8 stub:
  ...
  - Note: if Phase B produces no new architectural decision topic for this slot, update this entry to "Retired — no Phase B topic materialized; AD-M6-7 covered probe design; no further doc-Spec decisions identified."
...
# AC-10 (TG-DOCS-B): AD-M5-11 and AD-M6-7/8
grep -q "standards/autonomous" aria-orchestrator/docs/architecture-decisions.md \
  && grep -q "AD-M6-7" aria-orchestrator/docs/architecture-decisions.md \
