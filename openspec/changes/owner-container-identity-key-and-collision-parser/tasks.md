...
> **Spec**: [proposal.md](./proposal.md) (v8, Approved 2026-09-05) | **决策单**: [`.aria/decisions/2026-09-05-owner-container-identity-key-rulings.md`](../../../.aria/decisions/2026-09-05-owner-container-identity-key-rulings.md) | **审计**: post_spec R1–R5 + post_planning R1 聚合 `.aria/audit-reports/post_{spec,planning}-R*-…-aggregated.md`
...
> **Status**: A.2/A.3 **v2** (post_planning R1 rework 2026-09-05); post_planning R2 待跑
...
| S2 后续 (flip / 发布门 / a1-entry SC-3 改写 / #135 时间线复现) | **激活后追加为 6.x checkbox; 未激活 ⇒ D 期 Step 7 tracker issue** | 归档门只认 checkbox; 追加不改既有编号 |
...
- [ ] 1.11 SC-9 代码侧回归锁 (**baseline-green, 非 RED**): `phase-d-closer/tests/test_fetch_gate.py` 加 `kind="cross_owner"` 字符串夹具; rule 1.54 为散文规则无求值引擎 ⇒ 其触发面由 3.4 的文档 token 断言承载
...
- [ ] 2.7 `lib/identity.py` 新增 `get_container_label()`; container-id 文件头注释 (`:126-140`) 改写 — 1.8 accessor 子句转绿 (S1 不动 `get_container_id()`)
...
- [ ] 2.9 `track_board.py` ⚪ 行渲染 (dedupe 前调用; 插入点 `:459-475` kind 分支旁) + label 并列显示 — 1.9 转绿
...
- [ ] 4.2 全套回归: `python3 aria/skills/state-scanner/tests/run_tests.py` (stdlib unittest, 仓内正规跑法; pytest 备选须 `/home/dev/.local/bin/pytest` + `sys.path` 顺序 scripts 后 state-scanner 在前, 防双 lib 包) 零回归; `phase-d-closer/tests/test_fetch_gate.py`; session-closer / phase-d-closer handoff 写入测试 — SC-7
- [ ] 4.3 主仓 state-check: **13 条全绿**; `plugin-cache-currency` 预期 STALE 直到 owner D 期更新插件 (SC-7 例外条款) — 在 5.7 主仓同步面之后跑

## 5. 发布 (顺序: 5.2 bump → 5.1 merge+tag → 5.3 双推核验 → 5.7 主仓同步面 → 4.3 → 5.6 PR → 5.5 回帖)
...
- [ ] 5.5 issue 回帖 (**5.6 merge 后、归档前由执笔容器执行**): #193 / aria-plugin#135 (缺口 3) 指向本 Spec + 版本; #174 补 ship 结果; ship 后关 #193, #135 留缺口 1/2
- [ ] 5.6 主仓 feature 推 origin → `phase-c-integrator` C.2.4 gate → PR → merge → github 镜像
- [ ] 5.7 主仓同步面 (子模块推送核验后): 两 gitlink + `VERSION` + `README.md` badge + `README.zh.md` / `README.ja.md` / `README.ko.md` 版本串 + `docs/architecture/system-architecture.md` §2.8 与 `docs/architecture/version-scheme.md` 版本行 + CLAUDE.md 版本行
...
| S2-3 | 改写 a1-entry SC-3 判据 (仅 #174 ack 后) | ack 留言 id 记录 |
| S2-4 | 复现 #135 08-13 时间线不再 `claim_not_found` | acquire 后加 label 再 release 成功 |

**激活规则**: 0.1 判 S2-candidate **且** #174 ack 已到 **且** 5.1 merge 尚未执行 ⇒ 追加 `6.1`–`6.4` checkbox + yaml TASK-027..030 (接入 5.1 前置), 并在 handoff 记录激活时点; 三条件任一不满足 ⇒ 维持 S1, D 期 Step 7 用既有 tracker 机制开 issue 记录 S2 后续 (先例 sibling-spec-probe #192)。
...
| SC-5 | 3.1 / 3.2 / 3.3 |
| SC-6 | 1.6 → 2.6 |
| SC-7 | 4.2 / 4.3 (含 plugin-cache-currency 例外) |
| SC-8 | 1.4 → 2.3 |
| SC-9 | 1.11 / 3.4 / 3.5 |
| SC-10 | 1.9 → 2.9 |
| SC-11 | 1.10 → 2.6 |
