...
> **Status**: Draft (**合并版** — 由两个并发 #133 Spec 对账合并而成, per owner 决策 2026-05-30)
...
**0. 【前置·共同依赖】collision 字段持久化** (sister R1 C1 — phantom-field fix): 把 `_classify_collision` 提升为**共享 helper** (输入 `tracks[]`, 输出 `{kind: none|cross_owner|self_multi_container, groups}`, 同 owner+container→none 排除 self-serial);`handoff_multibranch` collector 调用并**持久化** `tracks_multibranch.collision` (additive);`track_board` renderer 改读共享字段;**meta-fix** `layer-l-integration.md:23,69` stale 字段名 (防 phantom 复发)。**注 (sister R2-CARRY #1)**: 此项含 `_track_to_claim_record→reconcile_all` 链非纯"抽函数",Phase B 评估是否拆**独立 prereq Spec**;helper 应为新函数 `classify(tracks)` 非直接 promote。
...
**3. 【辅助·写前同步】phase-d-closer 收尾 fetch-gate (fail-soft advisory)** [吸收 sister 切口1]: D.1 写 UPM **之前**: default-branch 解析 (`symbolic-ref refs/remotes/origin/HEAD`→`origin/master`→`origin/main`, 沿用 sync.py:36-41) + **fresh fetch** (`timeout=30`, 独立于 1.16 的 30s 缓存 — 收尾时缓存已陈旧;fetch 失败仅复用 `coordination_fetch` error_kind enum + soft-warn, **不回显 raw stderr** 防 credential 泄漏, 不阻塞) + behind-check (复用 git.py:167 ahead/behind 手法)。**触发** (防假阳性 R1 I5): 仅 `behind>0` **且** (`collision.kind!=none` **或** behind commits 触及 UPM 文件 [`git log --name-only <old>..origin/<def>` ∩ `upm.source_file` 非空, R1 I4]) → advisory;触及 UPM → 强提示。纯 behind>0 → 静默 (防 prompt fatigue)。**fail-soft** ≠ C.2.4.5 fail-hard。
...
- **合并 re-validation**: 本 Spec 是 Approved(b-backbone) + 未收敛(a/c sister) 的合并 → (a)/(c) 部分应 focused re-audit 收敛 sister R2-CARRY。
...
- [ ] **AC-0 collision 持久化**: `tracks_multibranch.collision` additive 字段由 collector 持久化 (`{kind,groups}`, 同 owner+container→none); helper 配 unit test 跑**真实** `collect_handoff_multibranch` 输出 fixture (非手搓 schema); renderer 改读共享字段回归 0; `layer-l-integration.md` stale 字段名修正 (meta-fix, AC-5)
- [ ] **SC1a 哲学合规 (可机验)**: grep 无 auto-enable / 无 block-on-conflict; coordination opt-in; 离线 fail-soft
- [ ] **AC-2 切口2 检测**: iff `collision.kind!=none` 且 config `enabled==false` → advisory; **disjointness**: `enabled==true` 时切口2 不触发 (phase1_gate 处理), `==false` 时反之 (两态互斥可测)
- [ ] **AC-1 切口1 fetch-gate fail-soft**: D.1 前 default-branch resolve + fresh fetch (timeout=30, 失败 soft-warn 不回显 raw stderr 不阻塞) + behind-check; 仅 `behind>0 且 (collision!=none 或 触及 upm.source_file)` 提示, 触及 UPM→强提示; 纯 behind>0→静默
