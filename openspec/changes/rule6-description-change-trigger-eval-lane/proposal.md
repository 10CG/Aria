...
- OQ-3 20 条 query 未经 owner 审阅: 接受为 trigger 套件 v1, 还是 owner 先审再 T4。**推荐先审** (skill-creator Step 2 要求); 代价: 若改动 query, 基线数字与套件 v1 脱节, 需重跑被评与负控两臂, 并行约 11–23 分钟 / 约 10 美元。**默认 (未裁时)**: T4 不执行, 不让未审套件搭车进 `ab-suite/`。
