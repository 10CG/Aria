...
> **Status**: 📝 **Draft (rewrite v2 + R1-fix + C1/C2 落版 + rework r1–r3) — post_spec R2 REVISE 未收敛 → owner 方向裁定 2026-08-23 已下, 待 rework v3 (缩 scope)**: **裁定 1: C2 (iii) 撤销, 只采 (ii)** (heartbeat 挂 state-scanner 入口编排层; `STALE_TTL` 不改 — 实读证伪的理据见 §2.2「请 owner 复议」); **裁定 2: 方向 b 缩 scope** — §4 竞品 spec 探针 (R2 簇 M-1/M-5) 与 §1 「关联 Issue」字段可得性/抽取规则 (R2 簇 C-A/M-10) 各自拆成独立小 Spec, 本 Spec 主体只留 A.1 入口认领 + track-id 契约 (R2 簇 C-B/C-C 必须在此解), 由换人执笔一次性清 R1 editlist 残项后再 R3 (max_rounds 剩 2)。前置依赖 `linked-issue-normalization` 已于 2026-08-23 ship (v1.67.0), 不再阻塞。R2 聚合: `.aria/audit-reports/post_spec-R2-1787481000000-a1-entry-claim-rewrite-aggregated.md`。
