...
> **本文件由脚本重新实跑生成, 不是从对话里抄的** (memory `pasted-evidence-is-derived`: 贴进文档的输出是派生物, 每次编辑后须由脚本重生成)。
> 生成时刻 (UTC): 2026-09-08T10:15:48Z · 主仓 HEAD `7cf81a3` · aria 子模块 HEAD `2b67ac6`
> 📌 **本文件不在 D-V2 / SC-12 的扫描范围内** (那两条逐字只覆盖 `proposal.md` 与 `tasks.md`)。
> 原因: 本文件收录 `check_bare_issue_refs.py` 在**正控版本**上的输出, 而那份输出按设计就是
> 一串裸 issue 引用的清单 —— 拿检查器扫自己的取证输出必然恒红, 且那个红零信息量。
> 如实登记而非把它加进允许清单 (加进去会让**未来真的写错**的同形状字面一并免检)。

> ⚠️ 落地复审抓到: C-3/C-8/C-10 三项验收逐字写「留证」, 但此前仓内**零输出** —— 勾是凭对话里的运行勾的。本文件补上。

## C-3 — `skill_md_literal_sync_probe.py` 四态

### 态 1/4 基线 — 期望 FAIL rc 1 且打印两侧原文

> B-1/B-2 已落地, 真仓现为目标态 ⇒ 基线用 **`aria` 子模块 `origin/master` 版**的两个目标文件
> 在同构插件树里复现 (探针本体用当前版, 只回退被测对象)。
...
### 态 2/4 目标 (真仓, B-1 落地后) — 期望 PASS rc 0
```
...
### 态 4/4 坏实现「两侧同改回 Step2」(同构插件树) — 期望 FAIL rc 1
```
...
## C-10 — `check_bare_issue_refs.py` 三态

### 态 1/3 目标态 (当前两份 Spec + handoff) — 期望 rc 0
```
裸 issue 引用: 0
rc=0
```
