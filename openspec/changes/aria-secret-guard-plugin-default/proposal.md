...
| **Risk — Known False-Positive** | `cat <script> && grep .env <script>` conservative regex 误拦 (parent DEC §4.3)。v1.24.0 **不修** (放宽 regex 引入新风险大于收益), changelog 显式 list known limitation **全集**: (a) `cat && grep .env` false-positive; (b) log-file grep 不在 risky_patterns 内的 false-negative (parent DEC §2.6); ack 路径足够日常应对 |
