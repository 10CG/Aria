...
| S7_HUMAN_GATE | 人类门控 | 普通 (M2 桩) | Feishu webhook 通知 owner; M2 = 发送即终, M4 = 等真 ack | **owner 单角色 (continues AD-M0-9)** |
...
CREATE TABLE IF NOT EXISTS dispatches (
  ...
  UNIQUE (issue_id) WHERE state NOT IN ('S_FAIL', 'S9_CLOSE')
);
...
#### 6.1 silknode 集成契约 (verbatim, per OD-3 + silknode-integration-contract §99)
...
**消费规则 (M2 实施期)**:
- M2 LLM 调用 (S2/S3/S6) 100% 经 silknode OAI baseURL `https://silknode.10cg.pub/v1/chat/completions` (具体 URL Phase A.1 末尾或 T0 owner 确认), 不直连 Anthropic API / 智谱 API
...
- S5_REVIEWING 同一 dispatch_id 内 review prompt hash 命中可复用 cache (Aria 侧, 非 silknode 侧), 跨 dispatch 不复用
