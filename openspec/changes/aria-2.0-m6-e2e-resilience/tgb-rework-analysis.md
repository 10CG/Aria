...
| **durability / auto-recover** | WAL faults | committed data survives a crash via WAL; SQLite auto-recovers. NOT S_FAIL (the spec's own WAL-D row already admits this — it generalises to A/B/C). |
...
| **Infra-2** Layer-2 alloc SIGKILL | alloc `failed` / `ExitCode=137` → `alloc_status_provider` → S5_AWAIT routing → S_FAIL | `test_t2_alloc_status_provider` (`test_failed_with_terminated_event_extracts_code` = ExitCode 137 = SIGKILL; `test_lost_maps_to_lost`) + T6.2 S5_AWAIT `running/terminated/lost/crash` routing | → S_FAIL ✅ | **model correct + already covered.** |
| **Infra-3** WAL truncation (A/B/C/D) | WAL **durability after crash** (committed data survives; SQLite auto-recovers) | `test_t22_t23_orm_wal.py` `test_57` (WAL durability after simulated crash) + `test_58` (reader/writer isolation) | A/B/C → S_FAIL ❌; D → recover ✅ | **model PARTLY WRONG + core covered.** WAL is a durability mechanism; A/B/C "→ S_FAIL" contradicts D's own "→ auto-recover". |
...
**Net**: all 6 modes are already exercised by M2/M3 + the one new handler test shipped this
session (`85b8f46`). The genuine previously-uncovered gap was exactly one branch (handler
`except Exception → S_FAIL(PROVIDER_5XX)` for non-timeout provider errors) — now closed.
...
3. **AdvancingClock DI / 100% coverage** (TG-B-statemachine) — re-scoped: `MockClock` already
   ...
   on a **4500-line** `extension.py` is a multi-day effort wildly off the spec's "~4h". This needs
...
| TG-B-statemachine: AdvancingClock DI + 100% cov of 4 modules (~4h) | **RE-SCOPE** with an explicit owner decision (see §3.3). MockClock already exists; 100% cov of 4500-line extension.py ≠ 4h. |
...
2. **TG-B-statemachine scope** (§3.3): (a) drop 100%-cov goal, (b) realistic threshold (e.g. cover
   the deterministic transition table only), or (c) changed-lines coverage? The "4500-line file →
