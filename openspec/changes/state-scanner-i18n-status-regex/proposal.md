...
| Pattern # | Before | After | 说明 |
|---|---|---|---|
| 2 | `^\*\*Status\*\*:\s*(.+?)\s*$` | `^\*\*Status\*\*[：:]\s*(.+?)\s*$` | 加 fullwidth colon |
| 3 | `^\*\*状态\*\*:\s*(.+?)\s*$` | `^\*\*状态\*\*[：:]\s*(.+?)\s*$` | 加 fullwidth colon |
| 4 | `^>\s*\*\*Status\*\*:\s*(.+?)\s*$` | `^>\s*\*\*Status\*\*[：:]\s*(.+?)\s*$` | 加 fullwidth colon |
| **6 (NEW)** | (无) | `>\s*.*\*\*(?:Status\|状态)\*\*[：:]\s*([^\|\n]+?)(?:\s*\\|\s*\|$)` | inline blockquote 多 meta |
...
更新 "Status 提取模式" 列表为 6 种, 加注 "中文 i18n 兼容: fullwidth colon `：` 与
半角 `:` 等价处理; inline blockquote 多 meta 行 (`> **A**: x | **状态**: y`) 由
pattern 6 捕获".
