...
> **Status**: ✅ **SHIPPED 2026-06-21** (aria-plugin **v1.49.0**, PR [#90](https://forgejo.10cg.pub/10CG/aria-plugin/pulls/90) merge `f3b7ac5` 双远程; standards `ddaf3d1` 双远程)。**Trigger B flip 执行**: telemetry 修复 (v1.46.5) 后, v1.47.0+v1.48.0 ship 的 gitlink commit 累积 **5 gate executions** (all warn-PASS) + tripwire **4 clean host-cron** + FP **0%** → owner **risk-accept sign-off** (透明披露: 5 条聚 2 ship 事件 + index.lock 重试虚增, 严格独立观察=2, owner 接受)。§A 3 处 default flip (submodule_gate.sh:33 + SKILL.md:450 + config/wording) + T-flip-12 测试 (unset MODE→block, 15 PASS) + §F standards wording。§B workflow cron 作废 (host-cron 已迁移 v1.41.0)。决策记录 `.aria/decisions/2026-06-21-v1.49.0-block-flip.md`。*(原 D+14 2026-06-07 DEFER → 2026-06-14 telemetry 根因修复 → 本日 Trigger B flip)*。
