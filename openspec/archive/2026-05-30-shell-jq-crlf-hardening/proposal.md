...
> **Status**: ✅ **Shipped** (aria-plugin v1.36.0, 2026-05-30) — Phase A.2 post_spec CONVERGED via challenge R1 (2 REVISE / 2 Critical) → Rev1 → R2 (code-reviewer PASS + qa REVISE / 1 NEW Major) → Rev2 → R3 (PASS). Phase B: 10 tasks done, 311 shell assertions PASS (Linux LF 零退化), crlf-shim framework 8/8 + jq-crlf-guard 7/7. Phase C/D: merged aria `0ab4c1b` (FF, feature branch) + standards `ec4924e` (convention) + main `0298d7b` (gitlink+VERSION+CLAUDE.md), 三仓双远程 parity verified. 实施前拦截 2 load-bearing Critical (C1 非空洞断言 + C2 content 正文篡改)。审计报告: `.aria/audit-reports/post_spec-R3-2026-05-30-shell-jq-crlf-hardening-consolidated.md`。Closes Forgejo Aria #132 follow-up.
