...
> **Spec**: proposal.md
> **Created**: 2026-06-25
> **Status**: ✅ **done** (shipped v1.50.0, 2026-06-26; Phase A R1→R2 CONVERGED + code-review I-1/I-2 fixed + Rule #6 AB +13.3pp owner sign-off)
...
- **Acceptance**:
  - [x] 脚本/测试落 `session-closer/`; `grep -rn session-closeout aria/skills/session-closer/` 无残留旧 skill 名 ([[feedback_verify_edit_landed_grep_count]])
  - [x] 旧测试 cherry-pick 后跑 (注意: 旧测试手造 fixture 假绿, 真接口修正在 TASK-003/004)
  - [x] 无 closeout_only/薄入口残留; §2.2.1 未拷入
...
- **Acceptance**:
  - [x] 无新建 `handoff-write-mechanics.md` 或第二份 handoff-write ref; 仓库仅一份 (AC-11)
  - [x] session-closer step4 + phase-d D.3 均 grep 命中引用 `handoff-mechanics.md` (AC-11)
  - [x] 若补段则 additive: 既有锚点 grep 仍在 (AC-8b); phase-d SKILL.md D.3 段 git diff 为空 (AC-8a)
...
- **Acceptance**:
  - [x] 独立可调, 不路由 phase-d, 不触发 D.1/D.2 (AC-1)
  - [x] leaf: 不调 phase-a/b/c/d/workflow-runner; 未归档 cycle 仅 advisory (AC-1b)
  - [x] step1/2 对话内省为 load-bearing 段 (AC-3); step2 主动提炼未固化经验 (AC-5b)
...
- **Acceptance**:
  - [x] grep phase-d description 不含「写 session handoff」与裸「收尾」(AC-9a)
  - [x] phase-d 执行逻辑 byte 不变 (AC-8)
...
- **Acceptance**:
  - [x] §7 用 `snapshot.sync_status.multi_remote` 嵌套路径填真实 SHA/parity + ahead 告警 (AC-2)
  - [x] §5 四维 UPM.cycle 读 `current_cycle` 非 None when configured (字段修正)
  - [x] 补漏: 静态输入对 (snapshot 有 followup X + 草稿无 X) → flag X (AC-3b, 机械单测)
  - [x] **真 snapshot 集成测试**: `four_dim_status` 喂真 snapshot 逐字段非空断言 (非手造 fixture, AC-7)
...
- **Acceptance**:
  - [x] `data_from_snapshot` 跑**真 snapshot** 集成测试: class-1 active_ids 提取非空 (字段修正后), 非 no-op (AC-4 adapter 子项)
  - [x] class-2 显式标 fixture-only (非静默退化); 4 类 fixture 命中 exit 0 (AC-4); drift guard 逐字段 (AC-7)
...
- **Acceptance**:
  - [x] §8 枚举 fixture (固定戳) count=2+表 (AC-5)
  - [x] step2 有主动提炼段, 非仅枚举既有文件 (AC-5b)
...
- **Acceptance**:
  - [x] 5 档 + 口径不混用 + unavailable 不报错 (AC-6)
...
- **Acceptance**:
  - [x] `grep -n 'session-closer/scripts/closeout_trigger.py' phase-b-developer/SKILL.md` 命中 ==1 (路径已重映射, 无 `session-closeout` 残留)
  - [x] phase-b/c step 显式调 trigger; 阈值 surface, 正常静默; 不引入 settings.json hook
  - [x] manual acceptance 验 occupancy≥阈值确实 emit (不凭文字 review)
...
- **Acceptance**:
  - [x] §2.2.1 重写 (grep 不含 `closeout_only` 委托措辞); standards 含消歧节 + 矩阵 (AC-9b/c)
  - [x] `grep -c 'session-closer' CLAUDE.md` ≥3 (nav + 两种收尾 note + Rule #9 L5); Rule #9 L5 含 session-closer 路径 (K-5)
  - [x] 架构文档与代码同步 (Rule #3); Parent US 回填 frontmatter
...
- **Acceptance**:
  - [x] (a) 触发命中率: 消歧矩阵词路由正确 (deterministic, AC-9c/AC-10a)
  - [x] (b) capability AB 跑出 + owner sign-off (delta≤0 不阻塞 ship, AC-10b)
...
- **Acceptance**:
  - [x] 6 面版本字面一致 (grep 验, 不信 commit msg)
  - [x] marketplace.json Skills count = ship 时现值 +1 (jq/grep 验)
  - [x] 双远程 parity 验证
...
|------|--------|----------|---------|-----------|
| TASK-000 | Done | tech-lead | - | - |
| TASK-001 | Done | knowledge-manager | - | - |
| TASK-002 | Done | knowledge-manager | - | - |
| TASK-002b | Done | knowledge-manager | - | - |
| TASK-003 | Done | backend-architect | - | - |
| TASK-004 | Done | backend-architect | - | - |
| TASK-005 | Done | backend-architect | - | - |
| TASK-006 | Done | backend-architect | - | - |
| TASK-007 | Done | tech-lead | - | - |
| TASK-008 | Done | knowledge-manager | - | - |
| TASK-009 | Done | qa-engineer | - | - |
| TASK-010 | Done | tech-lead | - | - |
