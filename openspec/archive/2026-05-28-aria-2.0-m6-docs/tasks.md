...
- [x] Confirm standards submodule is on `master` branch: `git -C standards branch --show-current` → must output `master`
- [x] Confirm no in-flight feature branch on standards: `git -C standards branch -a` → no `feat/` branch
- [x] Confirm aria/.claude-plugin/plugin.json version field: `python3 -c "import json; print(json.load(open('aria/.claude-plugin/plugin.json'))['version'])"` → record actual value for AC-3 badge check
- [x] Confirm CLAUDE.md current version: `grep "^\*\*版本\*\*" CLAUDE.md` → must show `1.0.4` (pre-TG-DOCS-A)
- [x] Confirm docs/architecture/system-architecture.md current version: `grep "^\*\*Version\*\*" docs/architecture/system-architecture.md` → must show `1.9.0`
- [x] Read `aria-orchestrator/docs/claude-md-revision-draft.md` fully — source for A.1 Diffs 1-8
- [x] Per `[[feedback_clear_cache_before_code_change]]`: if plugin reports unexpected behavior, clear Claude Code plugin cache before testing CLAUDE.md v2.0
...
**Subtasks**:
- [x] T-A1.1 Apply Diff 1: update top-level header block (项目本质 + 核心假设 + version line). Do NOT change 版本 number yet — that is Diff 9.
- [x] T-A1.2 Apply Diff 2: add §身份演进 (v1.x → v2.0) subsection before §研究目标.
- [x] T-A1.3 Apply Diff 3: add §两层 AI 分工 subsection after §十步循环 (live CLAUDE.md ~line 66). Max 20 lines. Layer 1: Hermes + Luxeno-routed GLM models = PM role. Layer 2: aria-runner + CC + aria-plugin = engineering role. Cross-link AD1 + AD6. <!-- R1-I3-4 fix: "Hermes + GLM" → "Hermes + Luxeno-routed GLM models" per 2026-05-21 redirect -->
- [x] T-A1.4 Apply Diff 4: add `aria-orchestrator/` row to §子模块职责 table. Add two §目录导航 entries (architecture-decisions.md + layer-boundary-contract.md).
- [x] T-A1.5 Apply Diff 5: add `✅ 实现 (v2.0)` line to §技术约束. Add clarifying v2.0 exception paragraph.
- [x] T-A1.6 Apply Diff 6 (NO-OP): verify Rules #1-#9 text is UNCHANGED. Run `git diff HEAD -- CLAUDE.md | grep "^[-+]" | grep -E "规则 #[1-9]"` and confirm no deletions or modifications to rule bodies. If any diff appears on rule bodies, ABORT and investigate.
- [x] T-A1.7 Apply Diff 7: add §Aria 2.0 运行时 chapter after §不可协商规则. Strictly ≤50 lines. Include: 分层叙述 (3-line ASCII), 与 9 条规则的关系 (table or list), 人类参与点 (S7), 详细入口 (4 cross-links).
- [x] T-A1.8 Apply Diff 8: update §项目状态. Set: 当前阶段 = "v2.0 M6 执行中 (M1-M5 shipped)". 成熟度 = 0.9. 插件版本 = actual value from `aria/.claude-plugin/plugin.json`. 主项目版本 = v1.7.0. Add 运行时版本 line. Update 更新 date at bottom.
- [x] T-A1.9 Apply Diff 9 (incremental): bump `**版本**: 1.0.4` → `**版本**: 2.0.0`. Update §项目状态 `插件版本:` field — live CLAUDE.md line ~434 shows `v1.22.0` (stale); read current value dynamically: `python3 -c "import json; print(json.load(open('aria/.claude-plugin/plugin.json'))['version'])"` and update to match (no hardcode). Verify Rule #7/#8/#9 body text is already present (no edit needed — they are already in live CLAUDE.md). Verify `phase_c_integrator.pre_merge_gate.no_aether_fallback` field name in Rule #8 matches `config.template.json`. Verify Rule #9 §2.3 cites `aria-plugin v1.22.x+` (already present; no edit if correct). <!-- R1-T3-3 fix: dynamic plugin.json read for §項目状態 version update; live CLAUDE.md shows v1.22.0 (stale vs v1.27.0 plugin.json SoT) -->
...
**Subtasks**:
- [x] T-A2.1 Update Plugin version badge: change `v1.15.2` → actual version from `aria/.claude-plugin/plugin.json` (read dynamically: `python3 -c "import json; print(json.load(open('aria/.claude-plugin/plugin.json'))['version'])"` — current is v1.27.0 per commit `c7e611f` but do not hardcode). Verify badge URL still resolves by inspecting badge format. <!-- R1-T3-3 fix: explicit dynamic read instruction; no hardcode -->
- [x] T-A2.2 Add Aria 2.0 positioning paragraph after §What is Aria? section: "**Aria 2.0 (v2.0.0, in progress)** extends the methodology to autonomous execution. See [docs/architecture/system-architecture.md](docs/architecture/system-architecture.md) for the two-layer architecture."
- [x] T-A2.3 Add cross-link to `docs/requirements/prd-aria-v2.md` in a suitable location (Roadmap subsection or Why Aria? section).
...
**Subtasks**:
- [x] T-A3.1 Create file. Add frontmatter: `# Aria v2.0.0 Release Notes` + date + status.
- [x] T-A3.2 Write **Plugin Compatibility** section (mandatory, closes brainstorm R1 cr-CH-8):
...
- [x] T-A3.3 Write **Highlights** section: M1-M5 delivery summary (Layer 1/2 architecture, dispatch loop, crash recovery, cost tracking, E2E resilience).
- [x] T-A3.4 Write **Migration Notes** section: "non-migration" — no breaking changes for interactive Aria methodology users. Autonomous runtime is an additive internal layer. Note TG-DOCS-B v2.0.1 possibility.
- [x] T-A3.5 Write **Known Limitations** section: TG-DOCS-B architecture docs may ship as v2.0.1 if calendar slips per Q-final-1 Menu C.
- [x] T-A3.6 Write **Forgejo Discussion FAQ** section (text to be posted by owner as Forgejo Discussion thread post-release):
...
**Subtasks**:
- [x] T-A4.1 Read existing `aria/README.md` to understand current content.
- [x] T-A4.2 Add cross-link section at a suitable location (near bottom or after introduction): "This plugin (aria-plugin) is the interactive layer of Aria. For the Aria 2.0 autonomous runtime, see the [main Aria repository](https://github.com/10CG/Aria) and [aria-orchestrator](../aria-orchestrator/)."
...
- [x] T-B0.1 Verify standards submodule is clean and on master: `git -C standards status` → clean working tree. `git -C standards branch --show-current` → `master`.
- [x] T-B0.2 Create feature branch in standards submodule: `git -C standards checkout -b feat/autonomous-docs`
- [x] T-B0.3 Create the `standards/autonomous/` directory: `mkdir -p standards/autonomous/`
- [x] T-B0.4 Implement T-B3 (create files in `standards/autonomous/`). Return here after T-B3 subtasks complete.
- [x] T-B0.5 Stage and commit in standards submodule:
  ```bash
  ...
  ```
- [x] T-B0.6 Push feature branch to standards remote: `git -C standards push origin feat/autonomous-docs`
- [x] T-B0.7 Owner action: merge the feature branch in the standards repo (via PR or direct push to master, owner discretion). Wait for merge confirmation.
- [x] T-B0.8 After merge, update local standards pointer to master:
  ```bash
  git -C standards checkout master
  git -C standards pull origin master
  ```
- [x] T-B0.9 Verify standards pointer is on master at the new commit: `git -C standards log --oneline -1` → should show the autonomous/ commit.
- [x] T-B0.10 Stage the submodule pointer bump in the main Aria repo:
...
**Subtasks**:
- [x] T-B1.1 Read the current file fully to understand existing structure and what must be preserved vs updated.
- [x] T-B1.2 Update frontmatter: `**Version**: 1.9.0` → `**Version**: 2.0.0`. `**Last Updated**: 2026-04-12` → `**Last Updated**: 2026-05-24` (or actual implementation date). Add `**Parent PRD**: [prd-aria-v2.md]` reference alongside existing prd-aria-v1.md reference.
- [x] T-B1.3 Update §Executive Summary: add two-layer AI execution model to "核心架构模式" list. Change "方法论研究项目" framing to include "参考实现". Add 自主运行时 as a new characteristic.
- [x] T-B1.4 Add new §Three-Layer Architecture section: methodology layer (standards/) / tools layer (aria-plugin) / runtime layer (aria-orchestrator). ASCII diagram: show Layer 1 (Hermes + GLM 5.1) and Layer 2 (aria-runner container + CC + aria-plugin). Cross-link to `aria-orchestrator/docs/architecture-decisions.md`.
- [x] T-B1.5 Add §Autonomy Model subsection: state machine overview (S0→S9 + S_FAIL). Human gate at S7_AWAITING_MERGE (AD10). Note crash recovery (M5 shipped, 3 infra modes + 3 LLM modes per Spec #2).
- [x] T-B1.6 Add §Layer Boundary section: note that `aria-orchestrator/docs/layer-boundary-contract.md` (B.4) pins the inter-layer contract. Reference cost.json schema locked at Spec #1 `c29a800`.
- [x] T-B1.7 Add §Version Scheme cross-reference: point to `docs/architecture/version-scheme.md` (B.2) for disambiguation of the four versioning streams.
- [x] T-B1.8 Review all existing sections for accuracy against M1-M5 actual state. Update any v1.x-only descriptions to note v2.0 evolution. Preserve all valid existing content.
- [x] T-B1.9 Final proofread: confirm Last Updated header is recent, Version is 2.0.0, all cross-links are relative paths.
...
**Subtasks**:
- [x] T-B2.1 Create file with frontmatter: title, created date, status.
- [x] T-B2.2 Write §Four Independent Versioning Streams table (per proposal.md §B.2 content spec). Include: stream name, repo/file, current version, SoT file, bump trigger, notes.
- [x] T-B2.3 Write §Why Streams Are Independent section: explain why aria-plugin version ≠ Aria main repo version. Emphasize semantic boundary (plugin = interactive tools; main repo = methodology + runtime).
- [x] T-B2.4 Write §Plugin Compatibility Clarification section: aria-plugin does NOT bump to v2.0. Cross-reference `docs/release-notes-v2.0.0.md §Plugin Compatibility`.
- [x] T-B2.5 Write §Version Lookup Reference section: for each stream, the exact command or file path to find the current version.
- [x] T-B2.6 Ensure ≥80 lines total.
...
**Subtasks**:
- [x] T-B3.1.1 Create file `/home/dev/Aria/standards/autonomous/decision-autonomy-matrix.md`.
- [x] T-B3.1.2 Add Lab-shareable header: `<!-- Lab-shareable: this file is in standards/ and may be reused by other 10CG Lab projects. See aria-orchestrator/docs/layer-boundary-contract.md for Aria-specific implementation. -->`.
- [x] T-B3.1.3 Write §Overview: what is an autonomy decision matrix, when to use it, PRD §553 source reference.
- [x] T-B3.1.4 Write the core decision matrix table: rows = decision types (triage / dispatch / merge approval / cost threshold alarm / schema migration / crash recovery escalation). Columns = autonomy level (fully-auto / auto-with-audit-log / human-gate-required). Each cell: condition that triggers this level + rationale.
- [x] T-B3.1.5 Write §How to Adapt for Your Project section (Lab-shareable guidance).
- [x] T-B3.1.6 Add cross-reference: "See `aria-orchestrator/docs/layer-boundary-contract.md` for how Aria implements this matrix in the Layer 1 / Layer 2 boundary."
- [x] T-B3.1.7 Verify ≥100 lines.
...
**Subtasks**:
- [x] T-B3.2.1 Create file `/home/dev/Aria/standards/autonomous/humanized-command-patterns.md`.
- [x] T-B3.2.2 Add Lab-shareable header: `<!-- Lab-shareable: this file is in standards/ and may be reused by other 10CG Lab projects. For M6 E2E corpus samples, see aria-orchestrator/evals/m6-prompt-quality/corpus/ (Spec #2 TG-C). -->`. <!-- R1-X-T1 fix: aria-orch/ → aria-orchestrator/ -->
- [x] T-B3.2.3 Write §PRD §639 Rubric section: **7 dimensions D1-D7** (SoT = Spec #2 §C.2; no divergence allowed):
...
- [x] T-B3.2.4 Write ≥10 curated pattern entries. Each entry:
  ```
  ### Pattern N: <name>
...
- [x] T-B3.2.5 Write §BOTH-Locations Design Note: "This file contains Lab-shareable curated patterns. The M6 E2E run corpus samples (raw data with per-sample scoring) are in `aria-orchestrator/evals/m6-prompt-quality/corpus/sample-{01..10}.md` (Spec #2 TG-C). These two locations serve different purposes and do not duplicate content." <!-- R1-X-T1 fix: aria-orch/ → aria-orchestrator/ -->
- [x] T-B3.2.6 Add cross-reference: "See also: `aria-orchestrator/evals/m6-prompt-quality/` for M6 E2E corpus samples and scores (Spec #2 TG-C deliverable)." <!-- R1-X-T1 fix: aria-orch/ → aria-orchestrator/ -->
- [x] T-B3.2.7 Verify ≥200 lines (rough proxy for ≥10 patterns at ~15-20 lines each + rubric section).
...
- [x] T-B3.3.1 Return to T-B0.5 and complete the commit + push + merge + pointer bump sequence.
...
**Subtasks**:
- [x] T-B4.1 Create file with Aria-specific header: `<!-- Aria-specific: this file is NOT Lab-shareable. It belongs in aria-orchestrator/docs/, not in standards/. Per km M-km-R2-005 decision recorded in AD-M6-9 (namespace creation decision, Q2 owner lock 2026-05-24). -->`. <!-- R1-X-T4 fix: AD-M5-11 → AD-M6-9 in header reference -->
- [x] T-B4.2 Write §cost.json Schema (locked at Spec #1 c29a800):
...
- [x] T-B4.3 Write §Humanized Command Format section: how Layer 1 issues YAML commands to Layer 2. Cross-reference `standards/autonomous/humanized-command-patterns.md` for curated examples.
- [x] T-B4.4 Write §State Machine Ownership Boundary table:
...
- [x] T-B4.5 Write §Error Escalation Protocol: Layer 2 failure modes (S_FAIL) → Layer 1 Feishu notification → owner optional intervention. Cross-reference aria-orchestrator/docs/architecture-decisions.md §AD-M5-* for crash recovery decisions.
- [x] T-B4.6 Verify `c29a800` commit reference is present and accurate.
...
**Subtasks**:
- [x] T-B5.1 Check if `aria-orchestrator/README.md` exists: `[ -f aria-orchestrator/README.md ]`.
- [x] T-B5.2 If exists: update to reflect M1-M5 shipped capabilities + M6 in progress. Add cross-links to `docs/layer-boundary-contract.md` and `docs/architecture-decisions.md`.
- [x] T-B5.3 If not exists: create minimal README with: title "Aria Orchestrator (Aria 2.0 Runtime)", description (Layer 1/2 architecture overview), current milestone (M6 in progress), cross-links to architecture-decisions.md and layer-boundary-contract.md.
- [x] T-B5.4 Verify `grep -q "Layer 1" aria-orchestrator/README.md`.
...
**Subtasks**:
- [x] T-B6.1 Add a new **AD-M6-9** entry (append after the last existing AD-M6-* entry or in the M6 section). Do NOT overwrite AD-M5-11 (it is reserved for M5-spillover topics at lines ~3460-3480 — leave untouched). AD-M6-9 content:
...
- [x] T-B6.2 Add AD-M6-7 entry documenting the state-checks probe design decision:
