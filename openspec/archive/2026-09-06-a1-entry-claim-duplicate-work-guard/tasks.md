...
- [x] 2.4 SC-2 / SC-8 / SC-29 (CLI 全链路 subprocess): 同 issue 不同 track-id 双方互含 (SC-2, baseline 即绿) / 终态 `done`·`abandoned` 带 `--include-terminal` 可见 (SC-8, baseline 必红) / 自排除两组 own=active 与 own=terminal (SC-29, baseline 即绿); 负控 = 删 `lib/collision.py:278-279` 两行验红 — ✅ **2026-09-05**: 新建 `tests/test_a1_entry_gate_cli.py` 8 用例 (SC-2 正臂双向 + 自排除负控 / SC-8 done·abandoned 两臂 + 默认路径守卫 / SC-29 两组), 全程 `--no-push` + tmp repo。**当前 7 红 1 绿, 是设计中的 baseline 红**: 红因逐字为 `phase1_gate: error: unrecognized arguments: --include-terminal` (exit 2) —— 该 flag 归 **TASK-014**, 而 `TASK-014 ← [003, 008, 013]`, 故本任务完成时 CLI 层必然仍红。唯一绿的是「不带 flag ⇒ 终态不可见」的默认路径守卫, 正好证明终态今天确实被 `:272` 丢弃。**⚠️ 欠**: Spec 要求的 CLI 层坏实现负控 (删 `collision.py` 自排除两行 / 门控丢 `--linked-issue` / flag=跳过全部 continue) 须等 TASK-014 后补跑, 现只在 lib 层跑了等价的自排除负控 (删两行 ⇒ 自己出现两次, 含终态那条)
- [x] 2.5 SC-24 / SC-33 / SC-25 代码臂 / SC-10 (CLI 全链路): `unknown_schema_claims` 计数且不入 overlap[] / `read_claims` 抛异常 ⇒ 双 `null` + error 非空 / overlap 抛异常 ⇒ `null` + error / fetch 降级 ⇒ `error == "fetch_degraded"` (四条 baseline 必红) — ✅ **2026-09-05**: 同文件加四组类 (SC-24 计数与不混入 overlap / SC-33 双 null + error / SC-25 null≠[] / SC-10 降级) + 四态可辨。**注入手段实测后钉 (c)**: Spec 列的 (a)「把 ref 指向非 tree 对象」**结构上做不到** —— `read_claims` 通篇 fail-soft, 损坏 ref 只得 `errors=['ls_tree_failed:fatal: not a tree object']` 而从不抛, 触不到 `except`; (b) 影子化 `lib` 单模块须重导出其余, 脆。钉 (c) = 子进程内模块边界替换, 且**按调用方栈帧限定** (`read_claims` 也被闸门自身 `_run_gate_impl:512` 调用, 无差别替换会把闸门打崩, 实跑一次即暴露 rc=1)。**五个坏实现负控全部实跑变红**: 删自排除 / include_terminal 跳过全部门 / 门控丢 `--linked-issue` / except 只赋 overlap / except 退回写 `[]`
- [x] 2.6 SC-23 / SC-14(a) (CLI 全链路, baseline 即绿回归守卫): A.1 原串 X acquire → `release_gate.py --raw-track-id X` ⇒ 不再 active; `--status abandoned` 臂; **补 SC-2 ↔ SC-23 相容性断言** (同一 `<slug>-<uuid>` 串既能 overlap 互见又能 release 命中; `:763-766` 待办 (3)) — ✅ **2026-09-05**: 同文件加 `TestA1CarryIdRoundTrip` 4 用例 (SC-23 跨 session release / SC-14(a) abandoned 臂 + 三选一 `parser.error` exit 2 / 相容性三步)。`release_gate.py` diff 为空。**发现并修补夹具保真度缺陷**: `_seed_other_container` 原先直接把原始串喂给 `acquire_claim`(它自己不归一, 生产里由 `run_gate` 先过 `derive_track_id`), 于是「改坏 `derive_track_id`」的负控只作用在本侧、种子侧纹丝不动 —— 首轮负控只打红 4 条, **SC-2 正臂与相容性用例没红**, 与 Spec「两条互为负控」的预期不符。夹具改为经 `derive_track_id` 归一后, 负控两半都点着 (SC-2 正臂 + 反向 + 相容性 + SC-8 两臂)。**相容性第 (3) 步作了一处适配并注明**: 原文「再跑 B 认领」需换容器, 而 CLI 身份恒为本机容器, 改用第三条串从本容器再认领, 被考察的性质不变
- [x] 2.7 SC-32 + SC-28 第二臂 + argparse 负控 (CLI 全链路): 无 carry-id 跑 `--heartbeat-only --phase A.1 --repo-path` ⇒ 遥测恰一条 `_source="heartbeat"` / `outcome="skipped_no_track"`, 不写 claim 不推远端; `coordination_probe` production 计数不变; 非 heartbeat 模式缺 `--raw-track-id` 仍 `parser.error` (baseline 必红: 模式不存在) — ✅ **2026-09-05**: 新建 `tests/test_heartbeat_only_cli.py` 7 用例 (SC-32 恰一条记录 + 不写 claim + 不推远端 / refreshed 臂 / SC-28 第二臂 production 计数不变 / argparse 负控 / push 三态)。断言对象取 `_telemetry_path(repo, "heartbeat")` 的返回值, 对文件名待裁三案免疫。**踩到并记下 memory `ss-two-lib-pkgs` 的一个新变体**: 常见的 `if p not in sys.path: insert(0,p)` 写法在此不可靠 —— `python3 -m unittest` 从 skill root 跑时 cwd 已以绝对路径躺在 `sys.path[0]`, 于是 root 那次插入被守卫跳过、只有 `scripts` 插到最前, 顺序当场反转, 报一个看不出所以然的 `ModuleNotFoundError: No module named 'lib.coordination_ref'`。改用 remove-then-insert (与 `phase1_gate.py:96-108` 的仓内惯例一致)。**三个坏实现负控实跑变红**: 只加 flag 不松 required / 只 `logger.debug` 不落盘 / 心跳写进 production 分区
...
- [x] 3.3 `lib/collision.py::linked_issue_overlaps` 增 keyword-only `include_terminal: bool = False` (默认路径逐字节不变; `:278-279` 自排除对终态同样生效) — ✅ **2026-09-05**: `linked_issue_overlaps` 加 `*, include_terminal: bool = False`; 只改第一道门 (`if not include_terminal and c.status in _TERMINAL`), 第二道门与自排除**未动**。lib 层五条直证: 默认路径取值不变 (`active` + `yielded`) / True 时 `done`·`abandoned` 现身 / `unknown` 两模式都被第二道门丢弃 (`linked_issue=None`) / `yielded` 不属 `_TERMINAL` 故两模式都在 / 自排除对终态同样生效; 既有 `TestLinkedIssueOverlaps` + `TestLinkedIssueNormalizationSC` 20 测试全绿 (三位置参数兼容)。**锚点位移 +13** (2 行签名 + 11 行 docstring): 短路 `:265→:278` · `_TERMINAL` `:268→:281` · 第二道门 `:274→:287` · **自排除 `:278→:291`** (TASK-007 负控指令引用的就是这两行, 补跑时按新行号)。**⚠️ verification 第 1 条「TASK-007 的 SC-8 臂绿」在本任务时点结构上不可满足** —— SC-8 是 CLI 层断言, 依赖 TASK-014 的 flag, 而 TASK-014 反过来依赖本任务; 该条实为 TASK-014 的验收项, 已记入 notes

## 4. `phase1_gate.py`

- [x] 4.1 第一处变更 ①②③④⑥: `--include-terminal` flag / `_main()` 调用处加关键字参数 / 门控放宽为 `if args.linked_issue or args.include_terminal:` + `read_claims` 只调一次 + `unknown_schema_claims` 键 / `except` 改写 `linked_issue_overlap = None` + `linked_issue_overlap_error` + `unknown_schema_claims = None` — ✅ **2026-09-05**: ①②③④⑥ 全落 + `:1154` 契约注释同步。三处 def 签名未触及, except 内字面 `= []` 零命中, 不传两 flag 时 stdout 键集与基线**逐项相同** (11 键实测比对)。**⚠️ 本任务 deliverable 与 SC-33 直接矛盾, 按 proposal 实现**: yaml 写「`linked_issue_overlap=None` **仅当传了 `--linked-issue`**」, 而 proposal `:623` 的 SC-33 明写「带 `--include-terminal` **不带** `--linked-issue` ⇒ 两键都 null」, Impact ④⑥ (`:663`) 亦为无条件「同一 except 分支须同时赋」; §2.4b 四态表 (`:340`) 进一步钉死「键缺席 = 未传 `--linked-issue`, **与是否传 `--include-terminal` 无关**」。⇒ 成功路径按各自 flag 填、异常路径无条件双 null。那句条件式是 A.2/A.3 派生时引入的错 (memory `derived-instruction-outranks-spec`)
- [x] 4.2 第一处变更 ⑤: `GateResult.error` 真正携带 `"fetch_degraded"` (Step 4 `health_check_fetch` 降级时; `:219` docstring 已预留) — ✅ **2026-09-05**: Step 4 降级记 `soft_error` (单点赋值), 只承接原本 `error=None` 的 **5 个**返回点 (4 处裸 `error=None` + 1 处条件式), 其余 6 个返回点本就带硬错 ⇒ 覆盖规则天然成立; docstring 写明优先级与「与 §2.4b `linked_issue_overlap_error` 是两条不同路径」。**⚠️ 爆炸半径超出 Spec 枚举**: 既有 `test_coordination_no_push.py:188` 断言 `error is None` 并注「主动跳过不是错误」, 而该类夹具**刻意无 remote** (类 docstring 明写) ⇒ Step 4 现在真降级, 该断言被我改变。处置 = 按其**真实意图**收窄为「不得是 push 侧错误」(`push_failed`/`auth_failed`/`max_retries_exhausted`/`user_aborted`), 并加注说明两条路径正交; 负控实跑确认收窄后仍有区分力 (把软错误伪装成 `push_failed` ⇒ 红)。**另**: SC-10 原夹具「不可达 remote」单独不成立 (无 remote 的仓同样降级, 删掉夹具也会绿) ⇒ 补对照臂「可达 bare remote ⇒ `error is None`」把它锚住
- [x] 4.3 第二处变更 ⑦ + `--heartbeat-only` 模式: `--raw-track-id` `required=False` + `_main()` 模式校验; 三级回落由编排层传入, CLI 不推断; 按 `(container, 归一 track_id)` 刷全部匹配 active claim; 遥测 `_source="heartbeat"` + `outcome ∈ {refreshed, skipped_no_track, skipped_disabled, error}`; 不写 claim / 不判碰撞 / 不自带 fetch; `coordination_probe.py` 口径注释 — ✅ **2026-09-05**: ⑦ `required=False` + `_main()` 模式校验 (落法 (a), 不用 subparsers); `--heartbeat-only` 模式经 `heartbeat_by_track` 刷全部匹配 active; 遥测独立 `source="heartbeat"` 分区 + `outcome ∈ {refreshed, skipped_no_track, error}`; push 复用 `resilient_push` 且受同一 `no_push` 门。verification 逐条实测: heartbeat 分支对 `health_check_fetch`/`acquire_claim`/`reconcile` 各 **0** 次调用 (不自带 fetch/不认领/不判碰撞) · `--heartbeat-only` 缺 `--phase` 仍被拒 · `--phase B` 缺 carry-id 仍 `parser.error` exit 2 · **空串 `--raw-track-id` 在 `derive_track_id` 之前短路** (`track_id: null` + `skipped_no_track`), R6 担心的「空串被归一成 sha256 去刷错 track」结构上不可能发生 · `coordination_probe.py` diff 仅注释行, `test_coordination_probe_cli.sh` 通过 · `test_phase1_gate_telemetry.py` 13 测试全绿 (production 分区不可被 heartbeat 写入)

## 5. 文本层 (SKILL.md / references / config / standards) — GREEN, 各依赖对应 6.x RED (R1 C1; 段落位置不变, 执行序在第 6 组之后)

- [x] 5.1 `phase-a-planner/SKILL.md`: frontmatter `:9` `allowed-tools` 加 `Bash, AskUserQuestion` + 独立标题级 `### 前置: REQUIRE claim (A.1, MUST)` 块 (放 `### 步骤执行` `:60` 之前; 七字面量 + 完整命令行 + 幂等谓词 + `--linked-issue` 两阶段取法含 `--emit-arg`) + A.1 YAML 项 (`:63`) 首键 `precondition:` 指针 (**委派动作行 = `:64` `skill: spec-drafter` / `:68-70` `action:`; `skip_if: complexity: Level1` = `:67`**, `:763-766` 待办 (2)) + §2.3 按 status 分档选项集 + §5.2 release 义务 + `coordination.enabled`/Level 1 零调用 + `unattended` 分支 — ✅ **2026-09-05**: frontmatter `:9` 逐字 = `allowed-tools: Read, Write, Glob, Grep, Task, Skill, Bash, AskUserQuestion`; `### 前置: REQUIRE claim (A.1, MUST)` 插在 `### 步骤执行` 之前 (切片 68 行, 内含七字面量 / 完整命令行 / 幂等谓词逐字 / 四态表含 `未能核实` / §2.3 按 status 分档选项集含「不要提议释放对方 claim」/ 退出义务两条 / skip 三条); A.1 YAML 项首键插 `precondition:` 指针。**④ 实测切片内不含 `--phase B`**
- [x] 5.2 `spec-drafter/SKILL.md`: frontmatter `:10` `allowed-tools` 加 `Bash` + 第二落点 `## 前置: REQUIRE claim (A.1, MUST)` 块 (放 `## 执行流程` `:73` 之前; 同款字面量 + 幂等谓词; 无 Level 1 例外) — ✅ **2026-09-05**: frontmatter `:10` 逐字加 `, Bash`; `## 前置: REQUIRE claim (A.1, MUST)` 插在 `## 执行流程` 之前 (切片 57 行); 幂等分工句写明只在「未经 phase-a-planner 直调」或「上游因 skip 未走到认领」时生效, **无 Level 1 例外**(直调路径无 Level 判定)。⚠️ 该文件是 **CRLF**, 用 `newline=''` 读写保住行尾 —— diffstat 60+/1−, 非整文件重写
- [x] 5.3 carry-id 三处占位措辞 (§2.1b, 逐字 `A.1 认领时派生的那一串`): `phase-b-developer/SKILL.md:92` + `:96-97` push 机制注释勘正 + `skip_if` 补「`--no-push` 只抑制推送不是 skip 条件」/ `branch-manager/SKILL.md:149` (命令行; R1/A4 实读, 原记 `:148` 是前一句; 标题 `:146` **不改**) / `phase-d-closer/SKILL.md:51-52` + `:55` (`:56` STALE_TTL 误写不改, follow-up) — ✅ **2026-09-05**: 三处占位措辞 + `phase-b-developer` push 机制勘正 (bootstrap 是 `push=False` @ `coordination_ref.py:800`, 真推送点 = Step 9 `:880` 与 7a self-resume `:597` —— **Spec 引的 `:856`/`:573` 是 d69091d 口径, 已被本 cycle TASK-014/015/016 位移, 此处写当前值**) + skip_if 补「`--no-push` 只抑制推送不是 skip 条件」; `branch-manager` `Part A1` 标题未动。⚠️ **首轮踩了 memory `preserve-crlf`**: `phase-b-developer` 与 `branch-manager` 在 HEAD 是 CRLF, Python `write_text` 静默转 LF 致 2133/1862 行整文件 diff; 回退后用 `newline=''` 重做, diffstat 降到 11+/2− 与 2+/1−
- [x] 5.4 `state-scanner/SKILL.md`: Layer L 段 (`:143-178`) 新增「Layer L A.1 heartbeat 集成」四句小节 (触发条件 / `--heartbeat-only` 完整命令行 / fail-soft / 指针) + `:168` 键集补 `push_skipped` / `push_skipped_reason` + `:176` 消费契约同步四态 (`list | null | 缺席`); 同批同步 `lib/constants.py:43-44` / `:50` 注释 (前提消失) — ✅ **2026-09-05**: `### Layer L A.1 heartbeat 集成` 四句 + 完整命令行 (与 layer-l 设计段**逐字节相同**, 已实测); `:168` 键集补 `push_skipped`/`push_skipped_reason`/`linked_issue_overlap_error`/`unknown_schema_claims`; 消费契约补三态 (缺席/[]/null+error) 与「不得用 `.get(key, [])` 读」。`constants.py` 的「NO production heartbeat loop exists」改写为落地后事实, **`STALE_TTL`/`SWEEP_TTL`/`HEARTBEAT_INTERVAL` 取值与不变量注释一律未动**
- [x] 5.5 `state-scanner/references/layer-l-integration.md` 四处: `:15` Design A 句同步 A.1 触发点 / `:45` `update_heartbeat()` → `heartbeat()` + caller/节律改真实 / 新增「Layer L A.1 heartbeat 集成」设计段 (三级回落表 / 遥测分区边界 / fail-CLOSED 谓词 `success == true ∧ coordination_ref_present == true` / 完整命令行) — ✅ **2026-09-05**: 四处全落 (Design A 补 A.1 触发点 / `update_heartbeat()` → `heartbeat()`+`heartbeat_by_track()` / caller 与节律改真实 / 新增设计段含三级回落表·遥测分区边界·fail-CLOSED 谓词·degraded 处置·13.8s 代价披露)。⚠️ verification 的「`git grep update_heartbeat aria/` 零命中」**结构上不可满足**: 守卫测试要断言该字面量不在文档里, 自己就必须含有它 —— 现 3 处命中全在 `test_coordination_default_lockin.py` 自身。该条应读作「除 guard 测试自身外零命中」
- [x] 5.6 `config-loader/SKILL.md` 登记 `coordination` A.1 skip 语义 + 新 key `state_scanner.coordination.unattended` (boolean, default false; 取值路径 = aria-runner 镜像内 `.aria/config.json`); `config-loader/DEFAULTS.json` 注册 `state_scanner.coordination.{enabled, mode, unattended}` 三键 (现状: `coordination` 整段缺席) — ✅ **2026-09-05**: `DEFAULTS.json` 注册三键 `{enabled: true, mode: "advisory", unattended: false}`; SKILL.md `enabled` 条目补「A.1 认领与 heartbeat 同受本开关」+ 新 `unattended` 条目 (type/default/取值路径 = aria-runner 容器镜像内 `.aria/config.json`) 且写明「**不得以 AskUserQuestion 是否可用做运行期推断**」(D15)
- [x] 5.7 `state-scanner/docs/coordination-ref-schema.md` §3.2 (`:129`) 追加第 6 条: `unknown_schema_claims` 语义 (独立键 / 措辞 / 不并入 overlap[] / 不与 done·abandoned 同档) — ✅ **2026-09-05**: §3.2 第 6 条追加 (独立 additive 键 / `int | null` 且 `null ≠ 0` / 不并入 `linked_issue_overlap[]` 的两条理由 / 不与 done·abandoned 同档 / 消费措辞 / 路径身份不提供)。§3.1·§3.3·§4.2 未动
- [x] 5.8 `standards/conventions/session-handoff.md` §2.3.8 (`:217`; **非** §2.3): 结构化 `{id, desc}` 之 `id` = 本 cycle carry-id = A.1 原串 (standards 子模块: 本地 merge + 双推 + 逐个 `ls-remote`) — ✅ **2026-09-05**: §2.3.8.1 schema 表 `id` 行「约定」列补一句 (走过 A.1 认领的 cycle, id = `<spec-slug>-<container_uuid>`, 与 §2.3.8.2 复用例外并列); 实测命中落在 `:229` 属 §2.3.8 区间, **不在** §2.3 机读 frontmatter 段; §2.3.8.3 硬约束段与 frontmatter 5 字段不变式均未受影响; diffstat 1+/1−。standards 已建 `feature/a1-entry-claim-duplicate-work-guard` 分支 — **✅ 2026-09-06 收口**: 先合 standards master (`d217ed0`, 同伴轨 §2.3.1/§2.3.5/§2.3.9 已 ship) 进 feature (`6c96a59`, `merge-tree` 试合并零冲突 —— 双方改同一文件的不同小节), 前置断言本地 master == origin == github 三者逐字节相等, **本地** `merge --no-ff` → standards master **`21748d4`** (第一父/第二父均实测相符), 双推后逐 remote `ls-remote` 独立核验 master + feature × origin/github 四项全一致; 主仓 gitlink 随之 bump `d217ed0`→`21748d4` (post-merge master SHA, memory `submodule_pointer_post_merge_bump`)

## 6. 结构化测试 (文本层 SC + rule6_note substitute, 均扩 `test_coordination_default_lockin.py`) — RED, 位于第 5 组上游; 同文件串行 6.1→6.6 (R1 C1)

- [x] 6.1 SC-22 ①–⑦: 两文件各自断言 (标题正则 + 围栏外 / 切片边界 `^#{1,4}[ \t]` / 七字面量 / 幂等谓词逐字 + `claims/` / 禁 `--phase B` / ⑤ `phase-a-planner` 切片外按 `A.1 - Spec 管理:` 锚点定位 yaml 围栏 (**实读 8 处 ```yaml, Spec 写 7**) / ⑥ 两条退出义务字面 / ⑦ 续行折叠后含 `python3 … phase1_gate.py … --phase A.1` 完整命令行); docstring 写明与 `test_phase_b_require_claim_present` (`:53-56` 裸 `assertIn`) 的强度差异是有意的 — ✅ **2026-09-05**: `TestA1RequireClaimBlock{PhaseAPlanner,SpecDrafter}` 两子类各自断言 (共享基类, **不拼接两文件文本**) + `TestA1PreconditionPointer` (⑤ 切片外)。围栏状态机: `_outside_fences` 清空围栏内行供 ① 求值, `_a1_block_slice` 以「围栏外的 `^#{1,4}[ \t]`」为边界(围栏内 `#` 不算), `_fold_continuations` 折反斜杠续行供 ⑦。基线亲跑 13 红, 红因逐条正确; ⑤ 的**围栏定位前提在基线就已通过**(找到带 `A.1 - Spec 管理:` 锚点的那个围栏, 只是指针未写), 证明不是抓第一个围栏
- [x] 6.2 SC-34: `phase-b-developer` / `branch-manager` / `phase-d-closer` 三文件各 ≥1 逐字 `A.1 认领时派生的那一串` (baseline 必红; 只改两处 ⇒ 红) — ✅ **2026-09-05**: 三文件 subTest 独立断言 + `Part A1` 标题未被误改的守卫。基线三文件 0 命中 ⇒ 红
- [x] 6.3 substitute (rule6 #6): `DEFAULTS.json` `state_scanner.coordination.{enabled, mode, unattended}` 三键与 `config-loader/SKILL.md:134`/`:140`/新登记值逐字一致 (baseline 必红); **负控** = 注册三键但 `unattended` 默认写 `true` / 漏 `mode` 键 ⇒ 红 (`:763-766` 待办 (4)) — ✅ **2026-09-05**: 三键集合相等 + 值逐字比 (`true`/`false` 与带引号字符串各自逐字)。基线 DEFAULTS.json 无 `coordination` 段 ⇒ 红; 原设计走 KeyError, 改为 `assertIn` 给出可读失败信息(同样红, 但读者不必从 traceback 反推)
- [x] 6.4 substitute (rule6 #10a): `layer-l-integration.md` 不含字面 `update_heartbeat` **且** 含 `heartbeat(` (baseline 必红); **负控** = 把 `:45` 改成 `heartbeat_by_track()` 却在别处新写一句 `update_heartbeat` ⇒ 红; 只删不补 `heartbeat(` ⇒ 红 — ✅ **2026-09-05**: 两条独立断言 (`update_heartbeat` 不在 ∧ `heartbeat(` 在)。基线 `:45` 含悬空名 ⇒ 第一条红
- [x] 6.5 substitute (rule6 #11): `coordination-ref-schema.md` §3.2 切片 (`### 3.2` 起至 `### 3.3` 止) 含字面 `unknown_schema_claims` (baseline 必红); **负控** = 字面写进 §4.2 而非 §3.2 ⇒ 红 (切片外) — ✅ **2026-09-05**: §3.2 切片断言。负控实跑: 把字面挪进 §4.2 ⇒ 变红, 证明全文 `assertIn` 对「写对了但放错节」完全免疫
- [x] 6.6 补充 substitute (rule6 #10b + #12): `layer-l-integration.md` 含标题字面 `Layer L A.1 heartbeat 集成` 且该节切片含 `--heartbeat-only` (baseline 必红; 负控 = 标题在、命令行在别节 ⇒ 红); `state-scanner/SKILL.md` `:168` 一带 (`### JSON 消费` 切片) 含字面 `push_skipped` (baseline 必红; 负控 = 写进 `## 相关文档` ⇒ 红) — ✅ **2026-09-05**: layer-l 新节标题 + 切片含 `--heartbeat-only`; state-scanner `### JSON 消费` 切片含 `push_skipped` / `push_skipped_reason`。基线两处均无 ⇒ 红
...
- [x] 7.1 照跑 `ab-suite/phase-a-planner.json` (当时套件全部 eval, d69091d 时 2; 零裁量; 前置 `ARIA_COORDINATION_NO_PUSH=1`; 结果落 `ab-results/<date>-<vNEXT>-a1-entry-rule6/`, `<vNEXT>` = 落地时按 plugin.json 计算, 不预写) — ⛔ **阻塞: AB 运行前置不满足** (见下方 §AB 运行前置) — ✅ **2026-09-06 照跑完成**: 6 evals (含 7.5 定向 4 条), with **25/25** vs old 13/25。结果 `aria-plugin-benchmarks/ab-results/2026-09-05-v1.70.0-a1-entry-rule6/phase-a-planner/`
- [x] 7.2 照跑 `ab-suite/spec-drafter.json` (当时套件全部 eval, d69091d 时 2 / 字段 Spec ship 后 3; 同上) — ⛔ **阻塞: 同 7.1** — ✅ **2026-09-06 照跑完成**: 6 evals (含 7.5 定向 2 条), with **30/30** vs old 17/30; 定向部分 14/14 vs 1/14。结果 `aria-plugin-benchmarks/ab-results/2026-09-05-v1.70.0-a1-entry-rule6/spec-drafter/`
- [x] 7.3 `ab-suite/state-scanner.json` 新增 eval (id = 当时 max(id)+1, d69091d 时 13) 钉点名行为 (d) (`enabled == true` 且持 active claim ⇒ 每次入口触发 `--heartbeat-only`; `enabled == false` ⇒ 零触发; SC-21 / SC-28 第一臂) 后照跑全部 eval (当前值 + 1); 同批程序化重算 `ab-suite/version.yaml` — 🟡 **套件编辑半已做 (2026-09-05)**: `state-scanner.json` 1.6.0→1.7.0 + new eval **13** `a1-heartbeat-on-entry-TARGETED` (三臂: enabled=true 给出完整 `--heartbeat-only` 命令行 / enabled=false 零调用 / `collision.kind` 为空不改变答案); `version.yaml` 1.4.0→1.5.0 **程序化重算** (实测 32 skills / 84 evals, 与旧 77 + 新增 7 吻合)。**跑评测半阻塞**, 见下方 §AB 运行前置 — ✅ **2026-09-06 跑评测半完成**: 13 evals 全跑, 新 eval 13 **with 6/6 vs old 2/6**。其余 12 个回归 eval 断言大面积恒真 (见 `aria-plugin-benchmarks/ab-results/2026-09-05-v1.70.0-a1-entry-rule6/DEFECTS.md` A 节)。结果 `aria-plugin-benchmarks/ab-results/2026-09-05-v1.70.0-a1-entry-rule6/state-scanner/`
- [x] 7.4 照跑 `ab-suite/phase-b-developer.json` / `branch-manager.json` / `phase-d-closer.json` (占位串 hunk, 各按当时套件全部 eval, d69091d 时各 2) — ⛔ **阻塞: 同 7.1** — ✅ **2026-09-06 照跑完成**: phase-b-developer / branch-manager / phase-d-closer 各 2 evals。**预测 delta=0 命中** (改动面与 eval 覆盖面不相交), 且发现两处「断言奖励错误行为」。结果 `aria-plugin-benchmarks/ab-results/2026-09-05-v1.70.0-a1-entry-rule6/{phase-b-developer,branch-manager,phase-d-closer}/`
- [x] 7.5 覆盖外档定向 fixture: `phase-a-planner.json` 增 (a) 拼串 + `--linked-issue` 省略/传递 (SC-9(A)(B) / SC-12 两臂 / SC-14(b)) · (b) overlap 非空请裁 + status 分档 (SC-11) · (c) fetch 降级/异常渲染「未能核实」(SC-25 行为臂) · (e) `unattended` 臂 (SC-26); `spec-drafter.json` 增 (a)(b) 两条 (新 eval id = 当时 max(id)+1, 不与字段 Spec 的 id 3 冲突; 不改任何既有 eval); 双臂可分辨; 同批程序化重算 `ab-suite/version.yaml` (串行于 7.3 后) — 🟡 **套件编辑半已做 (2026-09-05)**: `phase-a-planner.json` 1.0.0→1.1.0 +4 定向 fixture (id 3-6: 拼串与 `--linked-issue` 三态省略门含 Level1/enabled=false 子场景 / 按 status 四档请裁 / 降级「未能核实」与 unknown「已确认存在」不同句 / unattended 零 AskUserQuestion); `spec-drafter.json` 1.1.0→1.2.0 +2 (id 5-6, 直调路径同款去 Level 1 子场景)。每条都设了可分辨坏臂 (照传哨兵 ⇒ 复现 K8 / 渲染一行后自行继续 / 两者同句 / 用工具可用性做判据)。**跑评测半阻塞** — ✅ **2026-09-06 跑评测半完成**: 定向 fixture 合计 **with 45/45 (100%) vs old 16/45 (36%), delta +0.644**, 三套件各自成立。总账 `aria-plugin-benchmarks/ab-results/2026-09-05-v1.70.0-a1-entry-rule6/RESULT.md`
- [x] 7.6 套件缺口 issue: **新开** aria-plugin issue「phase-a-planner / spec-drafter 套件零覆盖 A.1 入口认领编排行为」, 正文交叉引用 `#117` (同族: 处方性 · 套件覆盖外) 与 `#127`, **不归并** (理由见 detailed-tasks.yaml TASK-036 notes) — ✅ **2026-09-06 开单 [aria-plugin#171](https://forgejo.10cg.pub/10CG/aria-plugin/issues/171)** (owner 授权; 依赖 7.5 已于 09-05/06 跑完)。**开单前 fetch 查重** (memory `concurrent_duplicate_audit_fetch_before_start`): 拉全 42 条 open issue 逐条看, 除 Spec 点名的 `#117`/`#127` 外**另发现同族 `#157`** (state-scanner 套件对 Layer L 段零覆盖) —— **不在原 A.2 裁量记述里**; 复核后维持「新开」(宿主套件与 SKILL.md 段均不同: #157 是 state-scanner 套件 × Layer L Phase B 集成段, 本 issue 是 phase-a-planner/spec-drafter 套件 × A.1 REQUIRE-claim 段), 并把三者边界与「若维护者认为应合并, 本 issue 可转评论」写进首段。⚠️ **两处按实测改写而非照抄本文件的 verification**: (a) 本行原写「两套件各 2 eval」—— 实测分支点 `788fac8` 是 phase-a-planner **2** / spec-drafter **4** (#117 的 2/2 是 2026-07-20 时点, 之后 spec-drafter 增至 4, 两处都对只是时点不同); (b) **现状已非 2 条** —— `5697477` 已把 7 条定向 fixture 写进固定套件, 两套件现各 **6** 条, 故 issue 按「落地前零覆盖 / 已做部分补救 / 待做通用维度」三段写。开单后独立 GET 回读核验: 正文 3404 字符, `#117`/`#127`/`#157`/`5697477`/`788fac8`/AB 数字/`unattended`/`v1.71.0` 全部命中 (不信 POST 回执)。

## 8. 文档 / 发版 / follow-up

- [x] 8.1 `aria/CHANGELOG.md` 条目 + 版本 SOT 5 文件同步 (`.claude-plugin/plugin.json` / `marketplace.json` / `VERSION` / `README.md` / CHANGELOG); 号 = `<vNEXT>`, **落地时按 plugin.json 计算, 本文件不锁字面量** (A.2 倾向 MINOR: 新 CLI 模式 + 2 flag + 2 lib API + 能力面扩权; **档位 = MINOR (2026-09-01 技术裁定, 决策单 §H1)**, 三份串行 ship 各占一号, 不合并一版; 号落地时按当时 plugin.json 计算) — ✅ **2026-09-06 落 v1.70.0**: 号按落地时 `plugin.json` 实测 `1.69.1` + MINOR 档算得 **1.70.0** (未预写字面); 五文件实 grep 核验一致 (`plugin.json` SOT / `marketplace.json` 两处 / `VERSION:3` / `README.md:5` / `CHANGELOG.md` 首条 `## [1.70.0] - 2026-09-06`), 两个 JSON `jq -e` 仍合法, 残留 `1.69.1` 仅在 `VERSION:5` 历史行。CHANGELOG 36+/0− 纯增, 不复述 SC 编号表, 只写用户可见行为 + Changed(allowed-tools 扩权) + Rule #6 三档 + 已知限五条。**写前逐项实读核验**: yaml 点名的 9 个交付物在 `7dd0135..工作区` diff 里全部有新增行命中; yaml 另提的「SC-32 文件名裁定」在 proposal 里 `文件名` **零命中**, 无法核实, 故未写入 (宁缺勿造)。
- [x] 8.2 主仓发版同步面 (与 086ee32 同口径 14 处版本点, R1 C2): gitlink `aria` bump 到 post-merge master SHA (index 条目, 非 `.gitmodules`) + 主仓 `VERSION:24` + `CLAUDE.md:139/:141` + root `README.md:8/:242` + i18n ×3 (`README.zh.md` / `README.ja.md` / `README.ko.md` 各 `:3` translated-from 标记 + `:10` badge + `:244` Plugin Version; 正文按 #140 B 档判是否重译, **标记与版本串必改**) → 全部改完后才断言机械兜底 `m6-version-badge-match` / `i18n-readme-translation-currency` 绿 — ✅ **2026-09-06 完成 (`2f8dd2b`)**: 前置先合 master (`b7aff46`) —— 同伴轨已把主仓 14 处同步到 1.70.0 且 master 前进 33 commit, 不合就是在陈旧基线上做 (memory `stale-local-main`); 唯一冲突 `docs/handoff/latest.md` 三处**逐一合表**而非二选一 (并补回对方丢掉的 `> ` 引用前缀)。gitlink `0545f86`→**`985e629`** (index 条目; orphan 守卫: 该 SHA 在 aria 的 origin 与 github 上实测均可达, `submodule status` 对 aria 无 `+`/`-`)。14 处版本点 → **1.71.0** (行号以 grep 实测为准, 非照抄本文件 2026-08-30 的旧行号; 实测与旧行号一致)。i18n 按 #140 **B 档**: 英文正文本次只动版本串无实质变更 ⇒ 不重译, 但 `translated-from` 标记与两处版本串同批改。⚠️ **另补 2 处本任务 deliverables 漏列的**: `docs/architecture/system-architecture.md` §2.8 与 `docs/architecture/version-scheme.md` 的 aria-plugin 版本行 —— CLAUDE.md 发布同步面明确含这两个文件 (PR #190 审计补入) 且有机械兜底 `plugin-version-arch-docs-match`, 但本任务清单未列; 改前该 check 实测**红** (DRIFT plugin=1.71.0 vs 两文档 1.70.0), 改后转绿。**属本 Spec 任务清单缺口, 建议后续 Spec 沿 CLAUDE.md 发布同步面而非 086ee32 的 14 处口径**。机械兜底 (全部改完后才断言): `m6-version-badge-match` OK badge=1.71.0 / `i18n-readme-translation-currency` OK (3 i18n @ 1.71.0) / `plugin-version-arch-docs-match` OK; custom_checks **13 pass / 1 fail / 14**。唯一未绿 `plugin-cache-currency` = 运行时插件缓存 (installed 1.69.1) 落后刚发布的 SOT, 发版后必然状态, 改仓内文件修不了, 需 owner 跑 `/plugin marketplace update` + `/plugin update`; 如实上报不作豁免 (Rule #10)。不带路径 `git status` 剩 `standards` (本轨改动待其 master 合并) 与 `aria-orchestrator` (他轨) 两个范围外 gitlink, 已如实记录非声称干净。
...
- [x] 8.4 aria 子模块本地 merge → master + 双推 + 逐 remote `ls-remote` 核验 + tag (CLAUDE.md 硬约束 1/2 的任务宿主; 两 remote 一致后才做 8.2 gitlink bump) — R2/A1 3221f943 残留补; **执行序 8.1 → 8.4 → 8.2** (编号不可变, 列于末不代表最后做; 见 yaml dependencies) — ✅ **2026-09-06 完成 (条款 1–5 逐条留痕)**: **条款 1 拦下一次真撞车** —— 本地 master 停在 `7dd0135` 而两 remote 已到 `0545f86`, 同伴轨 `owner-container-identity-key-and-collision-parser` 于 08:27:19Z 合并并 tag 了 **v1.70.0**, 占用了 8.1 刚写的号 ⇒ 按「号落地时按当时 plugin.json 计算」重算为 **v1.71.0**, 五文件同步改。先 `merge origin/master` 进 feature (`ec72175`, 非 rebase — 分支已推两端), 5 处冲突逐一解 (三处代码均为「双方各加一新函数位置相撞」⇒ 两存; phase1_gate 两处 import 取并集); **另修四处无冲突标记的静默错误合并** (双方都把 `1.69.1` 改成同一串 `1.70.0`, git 自动采纳不报冲突)。**条款 2**: 本地 `merge --no-ff` ⇒ master `0545f86`→**`985e629`**, 第一父 == 合并前 master、第二父 == feature HEAD `ec72175` (首次用 `-F -` stdin 致 exit 129 合并未发生, 断言如实报 ❌ 才发现 —— R4/A2 6bd70263 那条「断言本次而非既有双父」正是防这个)。**条款 3** owner 显式授权后推。**条款 4**: master 双推 + tag **定向**双推 (未用 `--tags`)。**条款 5**: 逐 remote `ls-remote` 独立核验 master / tag 对象 / tag 解引用 / feature 四项 × 两 remote 全一致 (首版核验脚本自身有 bug —— `ls-remote --tags <remote> <pattern>` 带 pattern 不返回 `^{}` 行, 用已存在的 v1.70.0 做对照证伪后修正)。测试: `run_all_tests.sh` 10 套件 OK / 0 FAIL / 0 SKIP / 2107; pytest 腿与合并前本分支比失败用例名集合逐条相同 (21 vs 21, 新增 0)。**条款 6 前提已成立**, 8.2 可 bump gitlink 到 `985e629`。
...
| **`scripts/phase1_gate.py` 全部锚点 (本 cycle TASK-014/015/016 累计)** | `d69091d` 口径 | **分区段非均匀位移**: `logger` `:61` 不动 · `GateResult.error` docstring `:220→:224` (+4) · `_takeover_eligible` `:299→:315` / `_run_gate_impl` `:351→:367` (+16) · 7a `:538→:562` / 7d `:762→:786` / `_telemetry_path` `:1010→:1034` (+24) · `_gated` `:1049→:1201` / `run_gate` `:1094→:1246` / `_main` `:1255→:1407` / `--raw-track-id` `:1268→:1420` / `--phase` `:1273→:1425` (**+152**) | TASK-015 的 soft_error 块 (+4/+16/+24 分段) + TASK-016 的 heartbeat 遥测器与模式实现 (+128 一次性插在 `_gated` 之前) + TASK-014 的 flag/门控。文件 1349→1566 行。后续任务引用 `phase1_gate.py` 行号须按所在区段查本行 |
| **`lib/collision.py` 门与短路锚点 (本 cycle TASK-013 引入)** | `:265`/`:268`/`:274`/`:278` | **`:278`/`:281`/`:287`/`:291` (统一 +13)** | 签名加 2 行 keyword-only 形参 + docstring 11 行; `def linked_issue_overlaps` 本身仍 `:230`。TASK-007 负控指令引用的「自排除两行」= 新 `:291-292` |
