...
3. **机械可查的红**: `grep -cE '^#### +TASK-014 验收判据' proposal.md` = **0** ⇒ 该小节尚不存在 ⇒ TASK-014 结构上无法开工。**这条今日就红** (已实跑 = 0), 且不预设 F 的任何形状。
   > ⚠️ **本条的第一版本身就是绿的, 当场自查抓到并换掉** —— 初稿写的是 `grep -c 'TASK-014 验收判据' proposal.md` = 0, 而本节这几段**正文自己就含这个串**, 实跑得 **> 0** ⇒ 它从写下的那一刻起就不可能红。⚠️ **此处刻意不写字面数字**: 2026-08-12 实测 **2**, 2026-08-13 复跑 **3** (此后本节又多一行描述该串) —— 该计数随本节正文增删而漂移, 而「> 0 ⇒ 恒绿」这个结论与具体值无关, 故 2026-08-13 起三处落点 (本行 · `tasks.md` TASK-014 段 · `detailed-tasks.yaml` TASK-014 verification (1)) 一律只记不等式 (memory `redfix-change-quantity`: 换量, 不在同一个量上调阈值)。换成**标题行锚定** (`^#### `) 后今日实测 0。留痕的价值在于: 这正是本 Spec 反复产出的那个形状 —— **判据与被判据的对象在同一份文档里, 描述它就满足了它** (memory `false_green_dual_is_permanent_red` 的镜像面)。
...
| **SC-M18** | 同一条承诺措辞 pattern 跑在删除面**其余四个文件**上: `grep -cE 'still (readable\|works)\|removed in v2\.0\|仍读\|v2\.0 移除' <f>`, `<f>` ∈ {`.../scripts/pre_merge_gate.py`, `.../phase-c-integrator/SKILL.md`, `.../tests/test_pre_merge_gate.py`, `.aria/config.template.json`} | **0 / 0 / 0 / 0** | **2 / 4 / 3 / 0** | 必红 (前三个)。⚠️ **本条是 post_planning R3/backend-architect 的闭合**: 上一版**只有 `config-loader` 一个文件**被提升为机械 SC (SC-M17), 其余四个文件只有散文描述今日计数、**无任何 `→0` 的断言** ⇒ 存在一个能通过 SC-M10/SC-M17/三条插入点 fixture/CLI 真实路径用例/TASK-021 终局复核**全部检查**、而 `pre_merge_gate.py:68`/`:116` 的 `will be removed in v2.0` **原样留在已发布的 v2.0.0 里**的实现。<br>⚠️ `.aria/config.template.json` 今日已是 0 (它只有键名面 `:75` `:78`, 无承诺措辞) ⇒ 该分量是**负控**, 守的是「TASK-020 不得往模板里补写承诺措辞」。<br>🔴 **2026-08-13 更正 (post_planning R4 遗留 `blocks_phase_b`; **同日 R5-fix 修订轮再改一次**, 上一版的更正自己是恒绿的 — 见下)**: 上一版此处逐字写「其**键名面**归零由 TASK-020 的中英并列枚举口径管辖, 不并入本 SC」—— 实读那条口径**只规定怎么枚举、不含任何期望值** ⇒ 那是一次**落空的移交**, 使删除面里**唯一受众在仓外**的落点 (采用方复制源) 成为全 Spec 唯一零机械断言的落点 (memory `delegate-verify`)。⇒ **键名面今日由 TASK-020 的一条独立机械腿承担, 单条命令四维一次判完**: `python3 -c "import json;b=json.load(open('.aria/config.template.json'))['phase_c_integrator']['pre_merge_gate'];k=set(b);C={'enabled','ci_backends','no_ci_fallback','wait_timeout_seconds','wait_check_intervals','primitive_call_timeout_seconds','poll_chunk_seconds','user_escape_hatch','path_coverage_enabled','_comment'};K=C-{'ci_backends','no_ci_fallback','path_coverage_enabled','_comment'};assert not k&{'no_aether_fallback','primitive_preference'},'legacy';assert k<=C,'unknown:%s'%(k-C);assert K<=k,'lost:%s'%(K-k);print('OK')"` **rc 须 = 0** —— 它同时断言 (i) 旧键名不在 `pre_merge_gate` 块内 (今日在 ⇒ 今日红) · (ii) **正向腿**: 块内每键 ∈ DEFAULT_CONFIG ∪ `_comment` · (iii) **内容量负控**: 六个既有键逐个仍在 · (iv) 仍是合法 JSON。八态对抗实跑 (3 好 + 5 坏, 含「保留块名只留 enabled」与「新键改名打错」两个上一版放行的坏实现) 见该任务。<br>🔴 **上一版这一格自己就是恒绿的 (R5-fix 修订轮实测, 逐字留痕)**: 上一版在本格写的是 `grep -cE` + 一个**表格转义后的竖线** pattern; 逐字复跑该式在**今日基线**返 **0** (同格却自称「2 → 0」, 单句自相矛盾), 在「删光承诺措辞、两个键原样留着」的坏实现上同样返 **0** ⇒ **放行** —— 因为 ERE 里 `\|` 是**字面竖线**不是 alternation。即 F-3 的修复在它自己新写的那一格里复现了 F-3 要治的假绿形状 (memory `fix-recurs-in-fallback`)。**本版判据整条不含竖线字符**, 故三份文件可逐字同形, 转义问题结构上消失。<br>⚠️ **同形的既存兄弟位置 (本轮普查新命中, 本轮不修 — 逐字声明)**: **本行 SC-M18 与 SC-M17 自己的 pattern 用的正是同一种转义**, 逐字复跑: SC-M17 式对 `config-loader/SKILL.md` 返 **0** (去转义后 **2**), SC-M18 式对 `phase-c-integrator/SKILL.md` 返 **0** (去转义后 **4**) ⇒ **这两行「今日值」列所写的 2 / 4 只有在读者自行去掉转义后才成立**; 照抄源文件的实施者会拿到恒 0。**本轮不改这两条承重 SC 的命令文本 = 触点配额** (改它须同批动本表 2 行 + TASK-001 红窗清单 + TASK-020 两条 verification + tasks.md 组 0 镜像), **不是判定它不成立** — 列为下一轮首选修项, 建议改法 = 换成 `grep -c -e PAT1 -e PAT2 …` 的无竖线形态。<br>⚠️ 该腿**刻意未编 SC 号, 且本版不再写下任何具体号** (编号须连带同步本表 + §测试基线计数句 + TASK-001 红窗 + TASK-021 终局**四处**, 本轮配额不足; 半编号 = 本 Spec 反复抓的镜像缺陷)。🔴 **修订轮更正**: 上一版一边这么声明, 一边把具体号逐字写进三份文件 (HEAD 基线三处均为 0), 机械枚举本文件得「被提到的号 − 有表行的号」多出该号 —— **那正是半编号本体**, 已全部去除。⇒ **本 SC 表今日确实不含它**, 「是否提升为编号 SC」列为下一轮首选修项 |
...
| **`CLAUDE.md`** | 规则 #8 那段须同步 (TASK-016 承接)。🔴 **2026-08-13 更正 —— 上一版逐字只写「新增第三条阻断腿」, 而那条腿 (分支存在性) 已依 `DEC-20260812-001 §2` 过户 A 侧, 本 Spec 自己的那条腿从未被这行提到**: 本 Spec 经 §6.1 引入的是 **legacy-key 硬失败**这条腿 —— 规则 #8 逐字 「无可用 backend 按 `no_ci_fallback` 显式降级」对**含旧键名的 config 这一输入类**被**条件性证伪** (硬失败钉在 `resolve_ci_backend` 之前 ⇒ 降级路径结构上不可达), 违 Rule #3 ⇒ 须同步。<br>🔴 **2026-08-13 R5-fix 修订轮补两点**: **(1) 只改本文件不够 —— SOT 那一跳同批** : 规则 #8 句末逐字「SOT: `aria/skills/phase-c-integrator/SKILL.md §C.2.4`」且 `DEC-20260731-001:15` 逐字把规则 #8 定性为「**精简指针**」; 实读该 SKILL.md `**降级行为**` 段 (`:293`-`:299`) 同属**无条件陈述**、被同一输入类同样条件性证伪, 而 SC-M18 今日四个命中落在 `:49` `:285` `:286` `:349` **无一在该段** ⇒ 只改本文件会把「文档 vs 代码不同步」换成「**指针 vs 它自己指名的 SOT 互相打架**」。⇒ **SOT 那一跳由 TASK-020 承接** (其 deliverables 本就含该 SKILL.md; TASK-016 不得把它加进自己的 deliverables, 否则打破 TASK-015 的结构不变量) —— ⚠️ **已去 TASK-020 核实**: 那条腿今日**逐字写着**按内容锚取节的 awk+grep 判据并带五态对抗实跑, 不是空口移交 (memory `delegate-verify`)。**(2) 分支存在性那半今日仍挂在 TASK-016 上, 其归属是 ⛔ 阻断式待 owner 裁定** (**五**选项 —— 含修订轮新增的 [5]「保留裁定记录但移出完成判据数组」—— 逐字见 `detailed-tasks.yaml` TASK-016 verification[1]); **裁定前 TASK-016 不得判 done**, 执笔方不代裁。先例: v1.31.0 CI backend 抽象化在同一提交同步过 Rule #8 (`commit 7661e96`) |
