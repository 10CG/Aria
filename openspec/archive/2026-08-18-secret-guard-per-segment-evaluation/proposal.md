...
>
> **规范要求 (三选一, 实现者自选, 但必须满足「段间零残留」)**: (a) 把 credit 计算封装成函数, `local has_filter=0` 开头; (b) 保持内联但在 `for seg` 循环体第一行写 `has_filter=0`; (c) 不用共享变量, 让每处判据直接把结果并入一个段内局部量。**验收 = SC-1 的第 2、3 条 fixture** (「风险段在后」与 Aria#170 本体) —— **急切版**粘性实现在这两条上必红; **最小版** (credit 仍挂 pattern 命中后) 至少在第 3 条 (Aria#170 本体) 必红。见 SC-1 反事实 (owner 2026-08-13 按实测厘清两变体)。
...
- **跨段 pattern fail-open (本版引入的第二类行为变更, 且是唯一 fail-open 方向)**: 逐段化后, 依赖跨 `;` / `&&` / `||` 上下文的 pattern 会失配, **今天拦得住的命令改后放行** (`2→0`)。**本版不兜底**, 但**必须申报 + 必须测量** —— 申报见 §Impact 行为变更段 (两类), 测量见 **SC-19** (12 条写死探针 + 57 家族全覆盖闸), 现状锁见 **SC-7** (四分判定表)。前一版这里写「SC-7 锁现状」是**事实错误**: 该形态的现状是 `exit=2`, 而 SC-7 要求的是改后 `exit=0` —— **SC-7 锁的是一个回归, 不是现状** (R6 silent-failure-hunter SFH-C2)。
...
> | 基线写法 | 受影响判据 | 注入点 |
> |----------|-----------|--------|
> | **严格**: `[[:space:]]*` 位置**不写**那个可选空格 (`cat x >/dev/null`) | **13 / 13** (owner 2026-08-13 采实测) | 23 |
...
> 3. **换行影响面** (`严格` 与 `宽松` 两基线): **owner 2026-08-13 裁定两基线均 13/13, 采 census 实测**。根因: 13 处判据每处都含 `[[:space:]]`, 而 POSIX `[[:space:]]` 含换行符 —— 任一处的空白位置注入换行都使 grep(逐行)/bash(整串) 分歧, 机械上必然全 13, 与「基线严格/宽松」无关。历史上「严格 11」依赖「只在基线命令已有空格位置注入」这一从未机械定义清的口径 (拿只触发 13 处里 1 处判据的 `cat x >/dev/null` 数「11 处受影响」本就对不上; 主 loop 用最接近口径复算得 10, 仍非 11), census 落地终结此争议。计数器仍输出两基线各一个数留审计, 但结论均 13。
>
...
- **行为变更 (穷尽声明, R6 修正为两类)**: 前一版写「**只有一类**」, **是假的** —— 它漏掉的那一类正好是唯一 fail-open 的那一类, 而这段是 Task 1.6 + SC-10 (CHANGELOG) 的**唯一输入** ⇒ 用户会收到「变严了, 请这样改写」的通知, 却收不到「有一批命令以前拦得住、现在放行了」的通知 (R6 silent-failure-hunter SFH-C1 + code-reviewer CR6-M1 **独立同现**)。
  ...
  | **第 2 类** | 依赖跨顶层 `;` / `&&` / `\|\|` 上下文才能匹配的 pattern, 逐段化后失配 | **`2 → 0` 放宽 (唯一 fail-open 方向)** | 工作面 = 82 条可跨段 pattern / **57 个家族** (转出 1 已定案的口径); v9 执笔席 25 条自造探针实测 **24 条为 `2→0`** | **不再被拦的形态说明** + 明确指向转出 1 —— **迁移写法对这一类没有意义** (用户不需要改写, 需要知道防护缩了) |
...
1. **[架构, 高]** 跨段 pattern fail-open — `set -o posix; set | grep foo` (2→0); `set -o posix && set | grep buildid` 同。工作面 = **1 条 `.*` + 81 条含 `[^|]` 的 pattern** 待逐条裁定 (其中 `[^|]*` **79** 条 / `[^|]+` **7** 条 / 两者兼有 **5** 条; 数组共 **141** 条 pattern)。
   ...
   >
   > **家族口径 (v9 新增, 交计数器)**: 把 82 条按**首个字面命令 token** 机械分组 (正则前缀里的 alternation 组整体算一个 key) ⇒ **57 个家族**, 最大的三个是 `ssh` 6 条 / `kubectl` 5 条 / `psql` 4 条。SC-19 的完备判据即以此为准 (每家族 ≥1 条探针), 分组由 `corpus_census.py` 输出, **不得手数** —— Task 1.4 + SC-18 承接。
...
- [x] 1.10 **ship 后执行 SC-9b** (✅ 2026-08-18 复验 PASS: cmp 1.66.1 字节相同 + harness 链拦截 #170 compound-credit 形态并逐段定位, 详见 detailed-tasks.yaml TASK-028 notes) (投递面腿, R5 qa-engineer M-2 指出 SC-9b 此前无 Task 承载): 版本 bump 落地并经 marketplace 刷新后, 用 `cmp` 比对 plugin cache 副本与 canonical, 再经 harness hook 链复验 ≥1 条本 spec 新增拦截形态。**挂 release-closeout, 不计入 Phase C 合并门槛** (理由见 SC-9b)
...
- [ ] SC-1 (baseline-failing, 核心; **承载 Task = 1.3 · 1.3b · 1.5 · 1.8**): 5 条泄漏形态 (`;` ×3 / `&&` / `||`) **改前 exit=0, 改后 exit=2**。**v9 逐条写死** —— 前八版只给形态不给命令, 而 R6 SFH-M2 指出「Phase B 自挑 5 条时完全可能挑成 5 条『风险段在前』的形态, 那样粘性 credit 就漏检」, 于是这条核心 SC 对 `has_filter` 未重置这个 fail-open 零鉴别力:
  ...
  - **反事实 (逐条写死; owner 2026-08-13 按实测厘清粘性两变体)**: 恒 fallback 实现 → 5 条全红 · **`has_filter` 不按段重置的粘性实现 → 视变体红不同条**: **急切版** (credit 每段无条件预算) → 第 2/3/5 条红、第 1/4 仍绿; **最小版** (credit 仍挂 pattern 命中后才算、但不清零) → **仅第 3 条红** (Aria#170 本体)。5 条对两变体合起来有完整鉴别力 —— 这正是「必须有风险段在后的 fixture」的理由; 若 5 条全挑成第 1 条那种形态, 两变体的粘性 bug 都抓不到。 · `split_top()` 漏 `\|\|` → 第 5 条红 · 漏 `&&` → 第 4 条红。
...
- [ ] SC-3 (拦截面不回归): **49** 条含边界的 `expected=2` 用例改后仍 exit=2; 任一转 0 = 安全回归。**有效面须机械确定 (owner 2026-08-09 采 R5 tech-lead R5-M-3 B-10)**: 「哪 49 条」**以 `corpus_census.py` 输出的用例名清单为准**, 不得手挑、不得凭「看起来含边界」判定 —— SC-2 已要求逐条列名, SC-3 的基数更大却无有效面定义, 是同一份 spec 内的不对称。该清单与 SC-18 断言的 `65/49/16` 同源
  - **⛔ 本 SC 对 `2→0` 方向结构上零鉴别力, 其全绿不得当作「无 fail-open」的证据 (R6 silent-failure-hunter SFH-C3 + code-reviewer CR6-M1 独立同现)**: 它名义上守「任一转 0 = 安全回归」, 但两席各自写逐段参考实现扫全语料, 都得到 `2→0: 0` —— **那是空集上的真空成立, 不是面小**。原因: `grep -n 'posix' secret-guard.test.sh` 无输出, spec 点名的唯一已知跨段实例根本不在语料里; 而可跨顶层 `;`/`&&` 的 pattern 有 **82 条 / 57 个家族**, 这 82 条的可达性**语料一条都没测过**。v9 执笔席另造 25 条探针, **24 条是 `2→0`**。⇒ **该方向的有效面由 SC-19 承担, 本 SC 只守语料内的 49 条**。两条 SC 各锁一个方向, 不可相互替代 —— 这是 memory `feedback_universal_predicate_vacuous_truth_on_empty_set` 在本 spec 的第二次现身 (第一次是 SC-11 对 credit 面恒绿)。
...
- [ ] SC-6 (fail-safe 降级族, **必须断言分支本身而非 exit code** — R3-M-1 + R4 qa-engineer §2 证 12 条里 5 条在「恒 fallback 的坏实现」下同样全绿, 因两路 exit 相同): 对每条**直接断言 `safe_to_split()` 的返回值** (fallback / split), 而非仅端到端 exit。**共 18 项** = 15 条端到端须 `safe_to_split=false` + **1 条隔离单元断言** + 2 条端到端须 `safe_to_split=true` (`ls -la; pwd` / `cat /opt/.env; echo hi >/dev/null`)。(16→17→18: R5.5 tech-lead TL-1 补 `!` 取反位置型一条 —— 给 v7 唯一改动的规范性实现文本 `BLOCK_KW_RE` 配可证伪的鉴别 fixture; v10 补后台记号型一条 —— §What.1 第 4 行判据此前没有专属 fixture, R1 tech-lead 判据 (e) 穷举反查抓到的缺口, fixture 与反事实表新增各一处见下)
  ...
  - **反事实 (逐类写死, v9 机械复验 `sc6_cf_v9.sh`; 分母一律 `/18`, v10 由 17 扩容)**:
    ...
    | 恒 split | **16/18** (全部端到端 false 项 + case 隔离断言 — owner 2026-08-13 采实测: 完整 stub 对裸 token `case x in` 亦返 safe, 隔离断言同红) | — |
    | 漏检 `until` | **1/18** | ✓ |
    | 漏检 `select` | **1/18** | ✓ |
...
- [ ] SC-7 (跨段 fail-open 的**已申报翻转**, `KNOWN-LIMIT`; **v9 按 R6 SFH-C2 重写**): `set -o posix; set \| grep foo` **与 `set -o posix && set \| grep buildid`** 两形态改后均 exit=0 (R3-M-4: 前三版只锁 `;`, 只处理 `;` 的兜底实现即可让本 SC 假性转红)。
  ...
  - 标注「归转出 1」。本 SC 只覆盖 2 条手写形态; **跨段面的测量由 SC-19 承担** —— 前一版把这 2 条当作整个跨段面的代表, 而实际工作面是 82 条 pattern / 57 个家族。
> 📌 **判据已变更 (owner 2026-08-24, 归档后勘正)**: 本 SC 的「增幅 ≤ 50%」相对闸已换成
> **绝对闸 `≤ max(100ms/call, 改前基线 × 1.5)`** —— 起因是 tier (e) 六次实测跨度 0.8%–83%
> 且最低负载那跑排第三高, 证明百分比在 34–48ms 小基数上不承载信息 (高负载同时污染分子分母)。
> 双腿设计 / 反事实矩阵 / 防绕过见 `.aria/decisions/2026-08-24-sc8-absolute-latency-gate.md`;
> 实现在 `aria/hooks/tests/secret-guard.test.sh` TASK-022 块。**下方原文保留不改** (归档件),
> 读时以决策文件为准。

- [ ] SC-8 (性能, **负载写死** — R3-C-3 指出前两版未定义负载致同一实现可在 +0% 与 +583% 间任选): **五档**负载各跑 20 轮, **判据取 min** (2026-08-16 主 loop 复核改, 原为「取中位数」; 理由见下方「统计量」子条), 改前改后同机同会话对比 —— (a) 单条 benign; (b) 2 段全 benign; (c) 2 段全命中 pattern; (d) 3 段全命中 (= 迁移建议的写法); (e) **最坏档 (本版补, 见下)**。判据: **五档增幅均 ≤ 50%**。§What.4 的 13 处转内建是达标前提。
  ...
  - **⚠️ 统计量 = min 不是中位数 (2026-08-16 主 loop 复核改, TASK-022 flaky 修复; 待 owner 过目)**: 原口径「20 轮取中位数」在本机高负载 (load 4-6 / 4 核共享机) + 小 N 下**flaky** —— TASK-022 agent 自跑 (e) 档 median +26.1% PASS, 主 loop 独立复跑同一测试即 median +53.4% > 50% FAIL (典型自测假绿, memory `feedback_agent_authored_tests_encode_own_bug_false_green`)。根因: median 受负载 spike 污染 (一半样本被调度打断), 改后 (e) 档 new_median 在 594k~723k µs 间乱摆。独立大样本 probe (N=60 × rounds=40) 定 (e) 档**真实增幅 min-based +11.8% / median +26.5%, 明确 < 50% 有大边际** —— 即真实性能达标, 53% 是纯测量噪声非退化。**min (rounds 轮里最少被调度打断的那次) 是性能对比的标准统计量** (代表纯计算成本, 去环境噪声), 小 N 下稳定。**判据阈值 (≤50%) 与负载档、轮数、进程内计时口径全部不变, 只把受污染的 median 换成去噪的 min; median 仍一并打印供审计对照。** 这不属 Rule #10 禁止的「不达标时换口径」—— 真实增幅已由 probe 确证 < 50% (达标), 换统计量是为消除 flaky 假 FAIL, 非绕过真 FAIL。owner 可回退 (改回 median 则须把 N 提到能压住本机噪声的量级, 实测 N=60 median 已稳定 +26.5%, 代价是回归耗时 6×)。
...
- [ ] SC-18 (**权威计数器自身被断言**, R4→R5 code-reviewer M-3 三轮未落): 跑 `python3 aria/hooks/tests/corpus_census.py --corpus <(git show <改前基点 SHA>:aria/hooks/tests/secret-guard.test.sh)`, 其输出与 spec 正文**逐数字机械比对**, 任一不一致即失败 ——
  ...
  - **v9 新增三面 (Task 1.4 第 (i)(ii)(iii) 项)**: 141 条 pattern 里含 `\b` 的 **16** 条 (R6 CR6-M3: 三种口径给 15/16/17, 与 `141→139` 同根源) · 82 条可跨段 pattern 的**家族分组表**, 家族数 **57** (供 SC-19 的完备判据比对) · 13 处 credit 判据的 alternation **28 分支 / 14 条零覆盖** (供 SC-15 维度 2 比对)
  ...
  - **⛔ 不达标时的处置路径 (v10 补, 与 SC-8 / SC-19 同款, Rule #10)**: 本 SC 此前是本表**唯一**没写失败出路的「有硬阈值却无失败出路」闸门。若实跑数字与本 SC 列出的正文数字不一致 —— **归键约定的歧义边界曾致家族数在 56/57 间摆动** (见 R1 code-reviewer M-3; **owner 2026-08-13 已裁定采 census 实测 57 + 写死「`\b` 视作停止字面 token 串」约定**, 本争议闭环) —— Phase B **不得**自行调整 `corpus_census.py` 的分组器实现去凑成正文写的数、**不得**反过来悄悄改正文数字迁就实跑值、也**不得**宣布「这条不适用」。唯一合法动作: 把 `corpus_census.py` 的完整实跑输出 (含具体分歧的家族/pattern/行号) 写进 handoff 请 owner 复议。
- [ ] SC-19 (**跨段 fail-open 面测量**, v9 新增 — R6 silent-failure-hunter SFH-C3 + code-reviewer CR6-M1; **承载 Task = 1.5 + 1.6**): 本 spec 引入的**第 2 类行为变更** (`2→0`) 必须被**测量**而非只被声明。SC-3 对该方向结构性失明 (语料是空集), SC-7 只有 2 条手写形态, 而工作面是 **82 条 pattern / 57 个家族**。
  ...
  - **完备判据 (硬)**: 探针集须覆盖 `corpus_census.py` 输出的**全部可跨段家族 (56 族), 每族 ≥1 条**; 计数器逐族比对, 任一族无探针即本 SC 红。上表 12 条**分属 12 个不同家族** (`ssh` · `kubectl` · `psql` · `tar` · `rsync` · `docker` · `base64` · `gh` · `GRP:(cat|head|…)` · `wget` · `python3` · `redis-cli`), **余 44 族由 Phase B 补齐** (56 − 12)。**反例条同样计入家族覆盖** —— 家族判据问的是「这个家族被测过没有」, 不是「这个家族翻转了没有」。
    > **⚠️ `printf` 族 owner 2026-08-16 豁免 (census 总族数仍 57, 但可跨段判据面 = 56)**: `printf` 族唯一 pattern (idx88 `printf -v NAME <进程替换读 env>`) 要求字面块字符, 而含块字符的命令**必被 `_sg_safe_to_split` 块字符判据降级走 fallback** (整条判定, 改后仍 `exit=2`, 主 loop 实测) —— 即 `printf` 族**结构上不可能跨段 fail-open**, 由 SC-6 块字符降级族间接覆盖, 不属本 SC 的风险面; 且其 pattern 要求块字符 ⇒ 无法构造 rule 5 合规探针 (2→0 探针不存在 / 反例探针须含块字符违反 rule 5)。故完备判据的分母由「全部 57 族」收窄为「全部**可跨段** 56 族」, `printf` 排除。self-check 仍断言 census `family_count==57` (验归键未漂), 但只对 56 可跨段族要求探针。此豁免的机读记录见 `.aria/notes/2026-08-12-secret-guard-128-phaseb-batch1-count-disputes.md §附 2`。
  - **⛔ 不达标时的处置路径 (与 SC-8 同款, Rule #10)**: 若 Phase B 判断 57 族全覆盖不可行, **不得**自行缩小家族口径、改判「部分族不适用」或删减探针 —— 唯一合法动作是把已完成的族覆盖率 + 未覆盖族清单写进 handoff 请 owner 复议。
