...
> **Status**: ✅ **SHIPPED 2026-06-07~08 (TG-1 v1.40.0 + TG-2 v1.41.0)**。Phase A.2 CONVERGED 2026-06-07 (post_spec R1 qa REVISE → Rev1 → R2 全票 PASS 3/3)。**TG-1 (R-fix-1)** = gate `submodule-gate-executions.jsonl` per-invocation 计数 (含 PASS) + PostToolUse `submodule-gate-telemetry.sh` hook (OQ1=(a′)) [v1.40.0, PR #76 `494b2f8`]。**TG-2 (R-fix-2)** = tripwire OQ2=(c) host-cron 迁移 (`submodule-tripwire-audit.sh` standalone + 坏 workflow DEPRECATED; dogfood 真仓库跑通首条成功 tripwire telemetry) [v1.41.0, PR #77 `b9b5d12`]。两 TG code-review PASS。**block-flip 机制层 unblock** (gate 记 executions + tripwire 可跑); 待攒 ≥3 真实 executions + tripwire 绿即可重启 block-flip (owner)。Closes 根因 R-fix-1 + R-fix-2。
