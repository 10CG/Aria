...
> **Status**: **✅ SHIPPED v1.58.0 (2026-07-16)** — aria-plugin `cae92e8` (gitlink 主仓 `de9abae`), 双远程 parity 验证。双动态工作流 review 通过 (code-reviewer **PASS** + silent-failure **PASS-with-fixes**, lint Return-denylist + docstring 诚实化 fold done, fix_introduced_regression: no)。1047 real-green + 1 AC-5 flaky。**Phase D 归档待**。Approved v5 (owner sign-off 2026-07-15) → **B.2 实现 (2026-07-16)**: `_common.py` 建 GitErrorClass (无 stderr 字段) + `_map_git_error_signal` (§3b signal 扩充) + `classify_git_error`; 5 直传 callsite (git.py×2 / sync.py×3) + 4 助手内部自分类 (handoff_multibranch×3 [_list_handoff_files benign-skip 保全] / handoff_worktrees×1) 全经通道; coordination_fetch._classify_error 委托 (措辞逐字节保 test_p1_layer_h.py:446); AC-2 best-effort lint `lint_stderr_typed_channel.py` (AST 追 `_run` 第三返回值, 自我否证抓 3 种泄露形态); 15 新测试 (AC-1 真失败/AC-1b timeout argv 结构/AC-2 lint/AC-4 标签集+委托 spy+signal+R6-m1/m4 守卫) + 全套 1046 real-green + 1 AC-5 flaky。**待 Phase B 对抗 review**。落地顺序 C→B→主 (C 已 ship v1.57.0); B 须先于主 spec F3′。
> **史**: v1 → R5 FAIL → v2 → R6 [1C+4M+5m] → v3 → R7 [1C fix-introduced+2M+2m] → v4 → R8 [1C fix-introduced+3M+1m] → v5 [option B 重框: 类型化通道=结构保证 / 助手内部自分类 / AC-2 降 best-effort / code-review 作完整性闸]。
