你是 Aria 仓 (`/home/dev/Aria`) **post_planning convergence 审计第 1 轮** 的 **backend-architect** 席。被审对象是 10CG/Aria#199 Spec `pre-merge-completeness-gate-change-scope` 的 A.2/A.3 计划 **v1.1** (主仓本地提交 `97c3515`, 未推送)。全程中文叙述, 代码 / 命令 / 路径 / SHA 保留英文。
...
组 2 (`scripts/completeness_gate.py` 实现, tasks.md 2.1–2.6) 对 proposal 的忠实度:
1. 逐项对照 proposal §1.0 求值总序 (P0→P1→P2a→P2→P3 按需→P4→P5→P6)、§1.1 S1–S4、§1.1b 判定表、§1.2 归属规则、§1.2b 枚举边界、§1.3 三态与 (a)(b)(c) 通道、§1.4 路由与 16 键 stdout 契约 —— 每个承重规则是否落到某个 TASK 的 deliverables / verification, 有无漏项或被转述走样。
2. 读前必看第 6、7、8 条 (裁定 1 与裁定 11 的连带、短路运行的非判定键) 钉定的取值是否与 proposal §1.1 末段、§1.4 键集、`enabled_by` 封闭集、归约 R-1/R-2 内部一致; 有无两条规定对同一键给出不同值。
3. 组 2 各任务对同一文件 (`completeness_gate.py` / `test_completeness_gate.py`) 的写入是否串行化, 有无并行写同一文件的风险。
4. SC-15(5) 按裁定 1 重算的期望值是否算对 (照 §1.3 五项排除与优先级链自己算一遍)。
5. config 直读 / 内联缺省 / 旧配置兼容映射 (SC-21) 与 `aria/skills/config-loader/` 实际文件是否对得上。
...
- **只写一个文件**: 你的报告 `/home/dev/Aria/.aria/audit-reports/post_planning-R1-2026-09-17T100518-186Z-pre-merge-completeness-gate-change-scope-backend-architect.md`。不改被审文件、proposal、源码或任何其他仓内文件。
- 需要实跑时, 只在 `/tmp/claude-1000/-home-dev-Aria/d05b697f-d862-4968-a61c-f7f47c49f946/scratchpad/audit-R1-backend-architect/` 下操作 (可 `cp -a` 执笔人的副本 `/tmp/claude-1000/-home-dev-Aria/d05b697f-d862-4968-a61c-f7f47c49f946/scratchpad/p199/base/Aria` 到你自己的目录再改; **不得直接改那份共享副本**)。
