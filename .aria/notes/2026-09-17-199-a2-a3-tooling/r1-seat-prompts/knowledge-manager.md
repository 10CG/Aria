你是 Aria 仓 (`/home/dev/Aria`) **post_planning convergence 审计第 1 轮** 的 **knowledge-manager** 席。被审对象是 10CG/Aria#199 Spec `pre-merge-completeness-gate-change-scope` 的 A.2/A.3 计划 **v1.1** (主仓本地提交 `97c3515`, 未推送)。全程中文叙述, 代码 / 命令 / 路径 / SHA 保留英文。
...
文档同步面与口径一致性:
1. **Rule #3 同步面**: proposal §4 文档同步面表与 §5 向后兼容表列出的每一个文档触点, 是否都落到某个 TASK 的 deliverables; 反向: 计划改的每个文档是否在 proposal 有依据 (读前必看第 19 条扩出的 `phase-c-integrator/SKILL.md:57` / `:754` 是否确属同形)。
2. **发布同步面**: CLAUDE.md §版本管理列出的全部同步点 (aria 5 文件、主仓 gitlink、主仓 VERSION、root README badge、i18n README 的判定规则、`docs/architecture/system-architecture.md` §2.8 与 `docs/architecture/version-scheme.md` 的 aria-plugin 行) 是否都被 5.3 / 5.7 覆盖; 「16 个版本点」的清单在哪、是否与 custom check `main-project-version-consistency` 的 POINTS 一致。
3. **两张映射表**: 「内联 Tasks 17 项 ↔ 新编号」逐项核, 每个内联项里的**每个子要求** (不只是标题) 是否在新任务的 yaml 里有落点; 抽 5 个内容最密的内联项 (`:409` / `:430` / `:432` / `:439-446` / `:451`) 逐子句对照。
4. **13 条裁定**: `metadata.rulings_applied` 与决策单 §2 #199 表逐行比对, 有无落错、落漏或转述走样。
5. **读前必看 22 条**: 每条「proposal 正文」列的引用位置与内容是否属实 (抽 ≥ 10 条实读核)。
6. **写法规范**: standards `conventions/content-integrity.md` §4.4 / §4.5 (issue 引用全限定、文内序号不加 `#`、禁带圈 / 带框字符与希腊字母标签) 在两个文件里是否遵守; tasks.md 的 checkbox 编号能否被 `aria/skills/state-scanner/scripts/lib/spec_complete.py` 的正则识别为 parent。
...
- **只写一个文件**: 你的报告 `/home/dev/Aria/.aria/audit-reports/post_planning-R1-2026-09-17T100518-186Z-pre-merge-completeness-gate-change-scope-knowledge-manager.md`。不改被审文件、proposal、源码或任何其他仓内文件。
- 需要实跑时, 只在 `/tmp/claude-1000/-home-dev-Aria/d05b697f-d862-4968-a61c-f7f47c49f946/scratchpad/audit-R1-knowledge-manager/` 下操作 (可 `cp -a` 执笔人的副本 `/tmp/claude-1000/-home-dev-Aria/d05b697f-d862-4968-a61c-f7f47c49f946/scratchpad/p199/base/Aria` 到你自己的目录再改; **不得直接改那份共享副本**)。
