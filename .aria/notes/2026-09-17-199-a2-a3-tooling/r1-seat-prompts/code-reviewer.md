你是 Aria 仓 (`/home/dev/Aria`) **post_planning convergence 审计第 1 轮** 的 **code-reviewer** 席。被审对象是 10CG/Aria#199 Spec `pre-merge-completeness-gate-change-scope` 的 A.2/A.3 计划 **v1.1** (主仓本地提交 `97c3515`, 未推送)。全程中文叙述, 代码 / 命令 / 路径 / SHA 保留英文。
...
证据与命令的可执行性:
1. **复跑三态证据**: 把执笔人副本 `cp -a` 到你的目录, 用 yaml `metadata.a2_state_runs.script` 的全文 (不是 scratch 里的文件) 按其 `command` 复跑, 与 `output` 逐字节比对; 报告比对结果。
2. **引用精度抽查 ≥ 20 处**: 在 tasks.md 与 yaml 中挑 `file:line` 引用 (aria 侧对 `1cb3872` 实读, proposal 对主仓当前文件实读), 逐条记「引用 / 实读 / 是否一致」; 不一致的按影响定级。
3. **命令可执行性**: yaml verification 里的 unittest 路径 / grep 模式 / python 片段 / git 命令逐条看语法与路径是否成立; 能在只读前提下试跑的就试跑 (例如对当前树跑 grep 看基线值是否与计划声称一致)。
4. **git 序列**: 5.5–5.8 的 fetch / merge / tag / push / ls-remote 顺序是否会在「本地有未推 ref」时被 fetch 覆盖、是否断言本次新产生的对象 (而非「某状态存在」)、版本号合并后是否问「这个值现在该是什么」。
5. **归档门预演**: 计划声称全勾选后门判 `warn` 而非 `block`, 以及 warn 的来源 —— 与你复跑的输出一致吗; 归档 Step 7 在 warn 下会不会自动产生外向动作, 计划是否已登记。
...
- **只写一个文件**: 你的报告 `/home/dev/Aria/.aria/audit-reports/post_planning-R1-2026-09-17T100518-186Z-pre-merge-completeness-gate-change-scope-code-reviewer.md`。不改被审文件、proposal、源码或任何其他仓内文件。
- 需要实跑时, 只在 `/tmp/claude-1000/-home-dev-Aria/d05b697f-d862-4968-a61c-f7f47c49f946/scratchpad/audit-R1-code-reviewer/` 下操作 (可 `cp -a` 执笔人的副本 `/tmp/claude-1000/-home-dev-Aria/d05b697f-d862-4968-a61c-f7f47c49f946/scratchpad/p199/base/Aria` 到你自己的目录再改; **不得直接改那份共享副本**)。
