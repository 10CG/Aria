你是 Aria 仓 (`/home/dev/Aria`) **post_planning convergence 审计第 1 轮** 的 **tech-lead** 席。被审对象是 10CG/Aria#199 Spec `pre-merge-completeness-gate-change-scope` 的 A.2/A.3 计划 **v1.1** (主仓本地提交 `97c3515`, 未推送)。全程中文叙述, 代码 / 命令 / 路径 / SHA 保留英文。
...
任务分解与流程设计:
1. 粒度: 每个 TASK 是否 4–8h 可交付、单一职责; 组 2 的六个任务切分是否与 §1.0 求值总序的边界一致。
2. 依赖与执行序: yaml `dependencies` 构成的 DAG 是否与 tasks.md 各组标题的执行序一致; 有无「后序任务读前序任务改的文件却无依赖边」「回归 / AB 测的不是最终树」。
3. 外向动作登记是否完整: 全计划每一个推送 / 开单 / 回帖 / tag / 协调 ref 写入是否都在 `metadata.owner_gates` 有对应项, 且授权方式与 owner 09-17 心跳裁定一致。
4. 发布段 (TASK 对应 5.5–5.8): 对照 CLAUDE.md 两条硬约束 (子模块本地 merge + 双推 + 逐 remote ls-remote; 禁服务端合并; gitlink 只前进) 逐步核; 对 `metadata.c25_five_questions` 去 `aria/skills/phase-c-integrator/SKILL.md` 与其脚本实读核验五问 (真做吗 / 方式合约吗 / 失败会红吗 / 何时触发 / 枚举哪些对象)。
5. 与 10CG/Aria#195 的顺序关系: B.1 等待点、版本取号、`ab-suite/version.yaml` 与 10CG/Aria#211 的撞号处置是否可执行。
6. AI 流程判断清单是否漏了计划里实际做出的流程性判断。
...
- **只写一个文件**: 你的报告 `/home/dev/Aria/.aria/audit-reports/post_planning-R1-2026-09-17T100518-186Z-pre-merge-completeness-gate-change-scope-tech-lead.md`。不改被审文件、proposal、源码或任何其他仓内文件。
- 需要实跑时, 只在 `/tmp/claude-1000/-home-dev-Aria/d05b697f-d862-4968-a61c-f7f47c49f946/scratchpad/audit-R1-tech-lead/` 下操作 (可 `cp -a` 执笔人的副本 `/tmp/claude-1000/-home-dev-Aria/d05b697f-d862-4968-a61c-f7f47c49f946/scratchpad/p199/base/Aria` 到你自己的目录再改; **不得直接改那份共享副本**)。
