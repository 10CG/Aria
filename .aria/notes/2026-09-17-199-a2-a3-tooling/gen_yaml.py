...
tasks = [
    ...
    T("TASK-017", "3.4", "旧 schema 散文残留三处、report-storage 四句、pre-write-validation 关联行", "S", "1-2", ["TASK-016"],
      ...
      [
          ...
          "frontmatter (含 description) 零改动 —— 本任务改的两份 SKILL.md 各做一次, 做法同 TASK-015: 以 awk \'NR==1&&$0==\"---\"{f=1;next} f&&$0==\"---\"{exit} f\' 切出首个 --- 与下一个 --- 之间的块, 对 git -C aria show <aria 起点>:skills/<name>/SKILL.md 与工作树文件各切一次, sha256 相等; phase-a-planner 直接切 (LF), phase-b-developer 先 tr -d \'\\r\' 再切。这两份与 TASK-015 / TASK-016 的两份合起来即本 cycle 被改 SKILL.md 的全集, 是 metadata.rule6_note.description_changed 取 no 的机械证据面 (TASK-018 复核)",
...
metadata = {
    ...
    "owner_gates": [
        ...
        "15 · TASK-001 / TASK-024 / TASK-031 与 Phase B–D 各会话调用 /state-scanner 之前 · 停下请授权 · metadata.coord_ref_precheck 退出非 0 (本地协调 ref 领先 origin 的提交含本轨心跳以外的写入, 或取不到远端值): 心跳加 --no-push 或本会话不调 /state-scanner, 不强制对齐, 不认领、不 release; 领先提交涉及的文件列表随请求呈上。v2.3 起本项另一触发条件 = 协调 ref 推后核验不过 (metadata.coord_push_verify: 心跳 / 认领 / release 返回 push_success 非 true, 或在不带 --no-push 的会话里 push_skipped 为 true, 或 ls-remote 与本地不一致且本地值不是远端的祖先): 不重试、不 force, 原样 JSON 与两侧 SHA 随请求呈上",
        "16 · TASK-001 / TASK-030 · owner 裁定 · metadata.commit_attribution 退出非 0: 待推送的主仓提交里含 foreign / foreign-merge (非本轨提交), 或含 shared-only (整条只落发布同步面、又没带本轨 Spec: trailer —— 典型是并发发版轨的 chore(release) 主仓同步面, 也可能是本轨 TASK-029 或 TASK-023 漏写 trailer —— 这两个任务的交付物结构上都不含 exclusive 路径): 逐条清单与 kinds 呈 owner; 未裁 ⇒ 不推送、不开 PR",
    ],
    "rule6_note": {
