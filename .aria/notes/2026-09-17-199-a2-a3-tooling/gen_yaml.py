...
tasks = [
    ...
    T("TASK-024", "5.2", "Rule #6 照跑 (两读法并集): audit-engine 与 phase-c-integrator 两个套件", "L", "5-8", ["TASK-023"],
      ...
      [
          ...
          "结束后先取第二次快照并完成上条比较, 然后才跑 metadata.coord_ref_precheck (它会 fetch); 退出 0 才按场景 1 第 3 步强制对齐 (被丢弃的只有本轨 --no-push 心跳); 退出非 0 ⇒ 不对齐, 停下请授权 (owner_gates 第 15 项); 对齐前后的值记台账; 换不带该变量的会话继续, 该会话第一步是 hard_constraints 第 4 条的会话入口 claim 核验 (对齐后解析不到 active 或心跳返回 claim_not_found ⇒ owner_gates 第 14 项; v2.5, post_planning R5 R5-M2)",
    ...
    T("TASK-029", "5.7", "主仓发布同步面: aria gitlink、16 个版本点、custom checks", "S", "2-3", ["TASK-028"],
      ...
      [
          ...
          "复跑 custom checks: m6-version-badge-match / i18n-readme-translation-currency / plugin-version-arch-docs-match / main-project-version-consistency / no-unresolved-version-placeholder 为 OK; plugin-cache-currency 在 owner 更新插件缓存前预期 STALE; 在主仓根跑, 其中 no-unresolved-version-placeholder 以 ! 反转 grep 的退出码并丢弃 stderr, 跑前先 test -d aria/skills 须退出 0 (做法同 TASK-027 第 7 步, v2.5)",
    ...
    T("TASK-031", "5.9", "Phase D: 开 issue、勾选、归档预演与 liveness 复跑、归档、释放 claim、回帖关单、周期 handoff、双推", "M", "3-5", ["TASK-030"],
      ...
      [
          ...
          "claim (D.2b): release 之前单独请 owner_gates 第 13a 项 —— 只含这一次协调 ref 推送, 不与 Phase D 提交双推同批 (v2.5, post_planning R5 R5-M1); 获授权后先跑 metadata.coord_ref_precheck, 退出 0 才跑 python3 -B aria/skills/state-scanner/scripts/release_gate.py --raw-track-id pre-merge-completeness-gate-change-scope --status done --repo-path <主仓根>; release 的推送按 metadata.coord_push_verify 核验 (released.success 为 true 且 push_success == true 且 push_skipped == false, 再 ls-remote 比对), 任一不成立 ⇒ 停下上报 owner_gates 第 15 项, 不重试、不 force。唯一例外: released.success 为 false 且 released.error 为 claim_not_found —— release_gate 把它记为 benign, 什么都不写也不推, push_success 为 null, 退出 0 (metadata.v2_state_runs 的 N11 实测) —— 不是推送失败, 不落第 15 项: 它说明本轨 claim 已不是 active (中途被 sweep 成 abandoned 等), 原 JSON 与三元组解析结果按 owner_gates 第 14 项的呈递口径呈 owner, 由 owner 裁「先按第 14 项重新认领, 再照本条重跑 release」或「不补认领, 记周期 handoff」(v2.5, post_planning R5 R5-M2)。未获 13a 授权或检查不过 ⇒ 不 release (不写仅本地的 release), 记周期 handoff",
          "10CG/Aria#199 与 10CG/aria-plugin#161 回帖 (版本号、合并 SHA、按 change 匹配与三态的行为变更要点) 后关闭 (owner_gates 第 12 项)",
          "周期 handoff 写 docs/handoff/ (Rule #9), 按模板 aria/templates/session-handoff.md 起稿 (phase-d-closer D.3 子步 2); frontmatter 放在文件最顶部, 五字段 track-id / owner-container / phase / status / updated-at 全写 (Rule #9 SOT standards/conventions/session-handoff.md §2.3.1, 取值域照该节), frontmatter 内不插注释行: track-id 逐字写 pre-merge-completeness-gate-change-scope —— 它同时是 metadata.commit_attribution 判该提交为 exclusive 的唯一依据 (写成别的或漏写 ⇒ 判 foreign, 停在 owner_gates 第 16 项); owner-container 逐字粘贴 python3 aria/skills/session-closer/scripts/handoff_autofill.py --owner-container 的输出 (handoff-mechanics.md: 机械填, 勿手动组装) —— 该命令失败时打印空串并退出 1 (handoff_autofill.py 的 --owner-container 分支), 粘贴前先看退出码: 退出非 0 或输出为空 ⇒ 按 handoff-mechanics.md 同段的回退, 照模板派生规则手填 (<owner> = git config user.email 的 @ 前段; <container-id> 按 standards/conventions/session-handoff.md §2.3.1 的三态), 回退事实记台账 (v2.5, post_planning R5 ac2e8dcb); 照录 tasks.md 的 AI 流程判断清单并追加 Phase B–D 新增项, 摘录 TASK-022 的活体输出与各 issue 号",
          "写后五字段自校验 (phase-d-closer D.3 子步 2b, 即 session-handoff.md §2.3.7 的 E1; 命令逐字照抄 aria/skills/phase-d-closer/references/execution-steps.md, 含 head -8 窗口): head -8 <handoff> | grep -cE '^(track-id|owner-container|phase|status|updated-at):' 须 ==5; 不足 → 按模板派生规则补齐后重验。不得带缺字段 handoff 进子步 3 (即下面两条的 latest.md 维护)。(口径注: 勿在 frontmatter 内插注释行, 可能把字段推出 head -8 窗口致误报。) 不得改写成对整份文件检索 —— 正文里的同名行会被计入而假绿 (2026-09-21 实测: frontmatter 只有 track-id、正文另有四个同名行时, 整份文件检索得 5, head -8 得 1)。E1 只验字段在不在, 本计划另加一次值非空检查 (不改 E1 原文): head -8 <handoff> | grep -cE '^(track-id|owner-container|phase|status|updated-at): *[^ ]' 须 ==5 —— owner-container 为空串时 E1 照样得 5, 此检查得 4 (v2.5, post_planning R5 ac2e8dcb)。输出原样记台账",
...
metadata = {
    "feature": SID,
    "title": "pre_merge Completeness Gate 加 change 维度 (A.2 / A.3 v2.5)",
    "level": 3,
    ...
    "hard_constraints": [
        ...
        "任何会推送协调 ref 的动作 (心跳、获授权的认领与 release) 与任何强制对齐之前, 先跑 metadata.coord_ref_precheck; Phase B–D 的每个会话在调用 /state-scanner 之前也先跑 (其入口心跳会推送, aria/skills/state-scanner/SKILL.md:182), 不通过则本会话不调 /state-scanner。v2.5 起每个会话的第一步是会话入口 claim 核验 (post_planning R5 R5-M2; 隔日续做、换会话继续都算新会话): 按 TASK-001 的 claim 身份条做三元组解析 → 协调 ref 前置检查条 → 心跳条 (它先把本地协调 ref 强制对齐到 origin 并在对齐后重跑解析, 再 --heartbeat-only, 推后按 metadata.coord_push_verify 核验); 对齐后解析到 0 条 active 或心跳返回 claim_not_found ⇒ owner_gates 第 14 项 (停在当前任务), 前置检查或推后核验不过 ⇒ 第 15 项; 入口核验通过前本会话不调 /state-scanner。理由: TASK-001 之后 claim 的存活原先只靠 /state-scanner 的入口心跳, 而它只在本会话已持 active claim 时才触发 (SKILL.md:182)、失败只记遥测 (SKILL.md:191); claim 超过 SWEEP_TTL (24h, lib/constants.py:58) 不刷新就可能被任一容器的 sweep 改成 abandoned。两处例外: TASK-024 的 AB 会话 (带 ARIA_COORDINATION_NO_PUSH, 心跳推不出去, 两次快照之间也不得 fetch) 不做入口核验, 由 TASK-024 的前置条让 AB 会话之前的最后一个普通会话补一次核验过的心跳、由其末条让 AB 之后的第一个普通会话先做入口核验; TASK-031 的 release 之后本轨已无 active claim, 此后的会话不再做入口核验, 心跳得 claim_not_found 属预期, 不走第 14 项",
    ...
    "owner_gates": [
        ...
        "17 · TASK-030 · 停下上报 · C.2.4.5 子模块指针闸 (aria/skills/phase-c-integrator/scripts/submodule_gate.sh, 本仓缺省 mode=block) 没放行: 退出 1 (block —— 某子模块的 feature 指针相对 origin/master 回退或分叉, 且无 override), 或退出 2 / 3 / 4 / 64 / 65 (闸没跑成), 或退出 0 却缺 TASK-030 该条要求的逐子模块结论行: 不在 Forgejo 合并, 原样输出随请求呈上; override (合并提交的 Submodule-Rollback: trailer 或 PR 标签 submodule-rollback-approved) 只由 owner 决定, 执行者不自行添加; 闸拿 HEAD 与 origin/master 的 gitlink 直接比, 同步合并之后 origin/master 又前进 (他轨 bump 了某子模块) 也会判 block, 是否重新同步后重跑同样由 owner 裁; 未裁 ⇒ 不合并 (v2.5, post_planning R5 R5-M3)",
    ],
    "rule6_note": {
    ...
    "v2_state_runs": {
        "what": "v2 新增的 N4 / C1 (cell_status) / N6 (coord_ref_precheck) / N7 (crlf_guard) / N8 / N9 (commit_attribution) / N10 (guard_config_hooks), 以及 v2.5 新增的 N11 (会话入口 claim 核验: 落后的本地协调 ref、强制对齐、claim 被 sweep 成 abandoned 后的心跳与 release), 在同一 scratch 主仓副本与临时仓上实跑; 代码取自本文件对应键, 副本外零写入 (N6 与 N11 的远端是临时裸仓)。v2.5 另改 N9 (加一态: 基准 ref 不存在 ⇒ 退出 2) 与 N10 (按钉在主仓根、看退出码的新 guard 重写, 加两态并并列 v2.4 命令的读法)",
...
header = ("# Generated by task-planner (A.3) — pre-merge-completeness-gate-change-scope, v2.5 (2026-09-22, post_planning R5 rework)\n"
