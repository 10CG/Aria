...
### 探针 Spec `sibling-spec-probe` B 期技术裁定 (2026-09-03; owner 选「1+2」= 探针 B.1 起 + aria v1.68.2 候选)

| # | 事项 | 裁定 | 判据 (可证伪) | 落点 |
|---|------|------|------|------|
| D1 | proposal §7 verdict 表第 1 行「`hits` 非空 ⇒ `sibling_found`」与 SC-3 字面「github fetch 失败 ⇒ `verdict=not_established` 且 hits 仍含 origin 命中」对同一夹具不可同时成立 (post_planning R1–R4 未抓; §7 是 P9 引入 `verdict` 一等字段后的承重条款, SC-3 沿用旧 SC-18 措辞) | **§7 表优先**: degraded 且有命中 ⇒ `verdict=sibling_found`, 覆盖缺口由 `status=degraded` + `reason=fetch_failed` 承载; 消费面 🔴 后追加「(覆盖不完整: <reason>)」。理由: 若取 not_established, 消费面按 §9 只渲染「未能核实」, 正证据在消费面消失 = SC-3 (iii) 自己禁止的「丢掉真实正证据」换了个面 | SC-3 测试断言 `status==degraded ∧ reason==fetch_failed ∧ verdict==sibling_found ∧ hits⊇origin 命中 ∧ exit 0`; 三负控不变 | 契约 `BRIEF-interface.md` §5 / execution-modes.md 契约节 verdict 表 / 测试 |
| D2 | v1.68.2 候选范围: 决策单 C7/C9/R5 六项中哪些随本 PATCH | **做**: `2ed89c8a` emit-arg reconfigure 回退 / rglob fail-open (含 symlink 两形态 + `changes/` 自身不可读) / archive 与在册 slug 目录不可 stat 的 traceback / `posixpath.normpath` + BOM。**不做**: hunk A 措辞软化 (B8, spec-drafter 处方性指令面 ⇒ Rule #6 AB 照跑, 与 M4 A.1.4 路径勘正合一批, 一次 AB) / 归档 proposal §4 六臂表 UNREADABLE 回写 (归档文档批次, 与 M1 同批) / 新 check 专属测试 C6 (主仓 state-check 基建, 非 aria) | 新增 6 条测试对 v1.68.1 探针的红数 ≥ 4 (见 CHANGELOG 1.68.2); 探针 SKILL.md 零改动 ⇒ rule6 substitute | aria `fix/probe-minors-v1.68.2` → 本地 merge + tag v1.68.2 (推送待 owner 授权) |
| D3 | 探针 Spec 与 v1.68.2 同一 aria 仓并行: TASK-014 断言 `git diff --stat -- skills/state-scanner` 为空 | v1.68.2 走独立分支先合 master 再把 master 合进 `feature/sibling-spec-probe`, 使 feature 相对 master 的 diff 不含 state-scanner; 两版本 (v1.68.2 PATCH + v1.69.0 MINOR) 同一次授权双推 | 合并后 `git -C aria diff --stat master -- skills/state-scanner` 为空 | aria 分支拓扑 |
| D4 | TASK-003「经 /skill-creator 产出」: skill-creator 的 eval 流程 = 写 evals (id/prompt/expected_output + expectations) → 双臂 subagent 运行 → grader → aggregate; 本 cycle 建文件阶段调用 `/skill-creator` 加载其 eval schema 并按其「Test Cases」段写两个 eval (descriptive 形态), 双臂运行留 TASK-017 | 留痕: 本决策单 + commit message; eval 文件字段与 skill-creator schema (id / name / prompt / expected_output / expectations) 一致 | `aria-plugin-benchmarks/ab-suite/audit-engine.json` |
| D5 | own_keys 为空时是否仍 fetch (Spec 只说「运行面一切正常 status=ok」) | **仍扫描** (status 如实反映覆盖), verdict not_established + reason own_token_absent; 不为省 ~25s 让 `status=ok` 变成未扫描的假绿 | SC-15 / TASK-008 reason 规则测试 | 契约 §5 |
| D6 | §6 cap 计数单位「(ref, path) 逐条累计」在本仓必触发 (origin 9 分支 ⇒ 1097 条 > 1000; 唯一 blob 仅 165) ⇒ 本仓每轮恒 `not_established` (恒红零信息) | **计数单位改为唯一 blob** (同 remote 内按 `ls-tree` 的 `<sha>` 去重, 同 blob 分类一次/计一次; 截断按枚举序丢首次出现的 blob); 数值 1000 不变; hits/去重/陈旧过滤/自命中全部保持 (ref, path) 级 | 双远端可达时本仓实跑 `status==ok ∧ caps_applied==[]`; SC-6/SC-23/SC-25 断言不变 | proposal Amendment A1 (append-only) + TASK-013 notes + 探针 `_scan_remote` |
