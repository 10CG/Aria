...
- 发布同步面: aria 子模块 5 文件 + 主仓 gitlink + 主仓 VERSION + root README badge + i18n README (**仅正文实质变更**才重译, #140 B 档) + `docs/architecture/system-architecture.md` §2.8 与 `docs/architecture/version-scheme.md` 的 aria-plugin 版本行 (PR #190 审计补入)。机械兜底: custom checks `m6-version-badge-match` / `i18n-readme-translation-currency` / `plugin-version-arch-docs-match`。
...
当前阶段: v2.0 M6 执行中 (M1-M5 shipped)。168h 自主跑三门未清:
  ...
  aria-plugin 方法论轨: v1.52.0–v1.68.1 已 ship — 逐版本史见 aria/CHANGELOG.md (SOT);
    残余 deferred 挂 Aria #168; 并发 in-flight track 见 docs/handoff/latest.md
版本: 插件 aria-plugin v1.68.1 | 主项目 v1.7.5 | 运行时 aria-orchestrator v2.0.0 (86bb684)
