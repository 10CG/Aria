...
当前阶段: v2.0 M6 执行中 (M1-M5 shipped; 成熟度 ~0.9 — 端到端验证 + 多终端协调 + 跨 30+ Spec + AB 累积)
  ...
  ⚠️ 168h 自主跑 ≠ make-ready: pre-flight (2026-07-02) 证自主 E2E 从未闭环 (数字-id dispatch 100% S_FAIL)。三门未清:
    ...
    - 遥测 Spec (AC-6 评分依赖, 独立) — **Drafting** (`aria-2.0-m6-cost-model-telemetry`, DEC-20260709-001 v2 + 4-agent 设计审议 + post_spec 5-agent R1→R2; 范围: Layer2 cost/model 遥测回报 + 容器模型接线统一 [smart-sonnet 清 + 单源 ANTHROPIC_DEFAULT_OPUS_MODEL + served-model 观测]; 红旗: glm-5.2 cutover 对容器疑 inert 待活体验); 容器侧 gate input-delivery 合并 (文件级重叠) + AC-10 gate Luxeno Blocker 4; Layer1 遥测管道可先独立。**依赖链** (input-delivery ↔ 遥测, disjoint 不互相包含): input-delivery 修 Blocker 3 (自主 dispatch 闭环 S9_CLOSE, AC-1); 168h 跑 AC-6 cost 维度可评分须遥测 Spec 独立 ship — input-delivery ship 不代表 168h 跑对 AC-6 可评分
...
版本: 插件 aria-plugin v1.55.0 (35 user-facing + 7 internal Skills, 11 Agents) | 主项目 v1.7.3 | 运行时 aria-orchestrator v2.0.0 (`f3848b2`)
