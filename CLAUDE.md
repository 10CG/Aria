...
```
当前阶段: v2.0 M6 执行中 (M1-M5 shipped; M6 Spec #1+#3 archived, **Spec #2 e2e-resilience 代码侧完成 2026-06-02**; **168h 运营跑受阻 — Layer1 runtime 降级 #147** [主 tick processed:0 空转 + gateway LLM 打 z.ai 死账户应走 Luxeno + Feishu 凭据失效 + 多处 stale deploy; 之前"只差启动 168h 跑"认知已纠正], Spec #4 sequential 待)
...
主项目版本: v1.7.2
运行时版本: v2.0.0 (aria-orchestrator master `dd52d34`, M6 execution phase;
          Spec #2 e2e-resilience TG-A+TG-B+TG-C 模板代码侧 100% ship via PR #23+#24;
          AC-6 pre-flight gate false-green 修复 PR #26 merge [#146, audit-engine 3 轮收敛 PASS])
PRD v2.0: Approved (2026-04-11) — M0-M5 done; M6 active (4 sub-Specs Approved 2026-05-24~25):
          ...
          → 填 corpus + 评分 → AC-5);
          **⚠️ 168h 跑前置阻塞 #147 (2026-06-29 live 核实)**: Layer1 runtime 降级 — 主 tick
          `aria-layer1-cron` 在跑但 processed:0 空转 (0 dispatch) + M1 handoff 缺失; gateway
          heartbeat LLM 打 z.ai 死账户 (429 余额耗尽, 应改走 Luxeno per owner); Feishu 凭据失效
          37 天 (人类门 AD10 失联); gateway 插件 + cron 多处 stale deploy (仓库有 register() 是旧版未部署)。
          AC-6 acceptance gate false-green 已修 (#146 PR #26); scan.sh 相对路径 exit 127 修复 PR #27 (#147 B5);
          deploy-doctor 初判经真核实纠正 4 处 (tick job 存在/register() 存在/系统在跑非"从未"/修复走 Luxeno 非充值);
