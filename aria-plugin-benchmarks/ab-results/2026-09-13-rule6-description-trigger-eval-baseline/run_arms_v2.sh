#!/usr/bin/env bash
# v2: 每臂独立项目根 (消除并发 worker 的合成命令文件互见) + 单 worker (同一根内不再有兄弟文件); 四臂并行
...
OUT=$S/trigger-results-v2; mkdir -p $OUT
...
run() { local name=$1; shift; local root=$S/trigger-proj-$name; mkdir -p $root/.claude; cd $root || return 2
  echo "=== $name start $(date -u +%FT%TZ)" >> $OUT/run.log
  if [ $# -gt 0 ]; then DESC=(--description "$1"); else DESC=(); fi
  python3 -m scripts.run_eval --eval-set $S/trigger-eval-openspec-archive.json --skill-path /home/dev/Aria/aria/skills/openspec-archive "${DESC[@]}" --runs-per-query 3 --num-workers 1 --timeout 120 --model claude-fable-5-1 --verbose > $OUT/$name.json 2>> $OUT/$name.err
