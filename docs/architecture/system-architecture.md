...
| 数据 | 格式 | 位置 | 更新频率 |
|------|------|------|----------|
| Aria 方法论变更 | OpenSpec (Markdown) | standards/openspec/changes/ | 按 Aria 改进 |
| **用户项目变更** | **OpenSpec (Markdown)** | **{project}/specs/** | **按功能** |
...
│  └── 仅限 Aria 自身的方法论改进 (如 aria-workflow-enhancement)  │
│                                                                 │
│  用户项目/specs/ 或 openspec/changes/                           │
│  └── 用户项目的功能变更 (如 user-auth-feature)                 │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘

原因:
1. aria-standards 是共享子模块，不应包含用户项目变更
2. Git 子模块特性导致变更被追踪到错误的仓库
3. 违背 Aria 作为"通用方法论"的初衷
```
