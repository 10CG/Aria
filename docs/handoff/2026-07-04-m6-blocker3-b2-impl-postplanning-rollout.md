...
phase: B
status: in_progress
updated-at: 2026-07-04T09:53:45Z
---
...
**代码位置**:aria-orchestrator `feature/m6-dispatch-input-delivery @ ef61f55` (B.2 批次 a3a4e2d + Critical 修复 ef61f55),**WIP 未 merge**,主仓 submodule 指针仍 `daf7c79`。主仓 master 有 doc-sync 交付项。

**Next**:owner 清 4 门 (⭐ 尤其 Blocker 4 Luxeno) → Phase C (PR+pre-merge gate+bump 指针) → Phase D 归档。AI-side 可先补 code-review 的 Important/Minor deferred 项 (见 §2)。
...
### 中优先级 (code-review deferred, AI-side 可做, 非阻塞)
- **[Important] AC-3 retriable/backoff 分支零测试** + `initial.sh` 注释指向的 `tests/initial-sh-unit/test.sh` 是**空未跟踪目录 (幽灵)**。retriable 逻辑审员手验正确但无回归护网。→ 写 `initial-sh-unit/test.sh` (source + override `_http_get_issue_raw` 返 503×N 断言 give-up) 或加 integration retry scenario;补 AC-2 bare-numeric-reject 断言。
- **[Important] 部署顺序耦合** (运维,非代码 bug):Layer1 (TASK-016) 先于新镜像 (TASK-010 marker) 落 → 旧镜像 file-mode SUCCESS 无 marker → outcome_class=UNKNOWN → 排除 verified corpus → AC-2 可能假 FAIL。→ deploy runbook 强制「镜像先、Layer1 后」或 168h 前置探针「running image 含 TASK-010 marker」。
- **[Minor] TASK-015 head_branch 测试是重言式** (`test_m6_dispatch_input_delivery_layer1.py::TestTask015...`) —— 但 `test_t_pr_timing.py` 另有真测覆盖 AC-5,重言式测试建议删/改。
- **[Minor] AC-7 sanitize pipeline 零自动化测试** (CRLF/截断/注入隔离),逻辑手验正确。→ 随 initial-sh-unit 补。
- 其余 minor:UTF-8 截断切多字节字 / base_branch fallback 未测 / migration 008 命中通用 backfill 规则 (预存模式) / exit0 双 get_alloc_logs (效率) / `<repo>` 分量取全局 env (单-repo 下正确, 多-repo latent)。
...
aria-orchestrator (feature/m6-dispatch-input-delivery, origin only, **未 merge**):
- `a3a4e2d` feat(m6): Phase B.2 — dispatch input delivery TG-1~TG-5 + TG-6 fixture (36 files, +3879)
- `ef61f55` fix(m6): render invariant Critical #1 ($ARIA_ token false-fire)

## §8 Memory entries this session (建议沉淀)
