...
## §16 Spec A post_spec R4 — max_rounds 走满, 八轮数据成图 (2026-08-12)

**3 REVISE / 2 PASS · 0C + 12M + 16m = 28 · 引入率 93% · 一席给出完整 PASS (零 findings)。**
aggregate: `.aria/audit-reports/post_spec-R4-1786559000000-premerge-gate-branch-existence-aggregate.md`
...
**⇒ 拆分买到了严重度天花板 (A 侧 Critical 连续三轮 0, B 侧从未做到),
没买到收敛; 且 A 侧引入率比 B 侧更高。**

R4 的 28 条里 **26 条是上一轮 fix 新造的** —— 这个循环现已**几乎纯自生成**。

### §16.2 🔴 编排层第 15 条错误 — 我漏执行了一条 owner 裁定, 且把它自行降级

**DEC-20260812-001 §5.3** (owner 裁定, Approved) 逐字要求「B 的 `detailed-tasks.yaml`
删去迁往 A 的任务时**须留 cancelled 痕迹**」。我只执行了 §5 的第 1、2 条;
第 3 条**既没做也没上报**, 被处置成「A 的 D.2 handoff 必写项」——
**把 owner 裁定的 A.1 迁移动作自行降级成了备忘**, 且 A 全文对 DEC **§5 零引用**
⇒ owner 在三个待裁点上**看不到它**。

后果 (席位逐字): B 的实施者拿到七条 pending 且规格已迁走的任务去实现
⇒ **第二份 `_verify_branch_exists` / merge conflict**。

✅ **已补执行**: B 的 TASK-003/004/005/007/008/009 标 `cancelled` + 逐条 notes 留痕
(⛔ 不得再实现 + 指向 A 侧承接) + metadata 追加说明。实测 **pending 15 / cancelled 6**, 一条未删。

**形状**: 与第 7、9 条同族 (**用 AI 的处置替代一条不该我降级的决定**), 这是该族第 **3** 次。

### §16.3 一条方法论上最值得记的

R4 复核执笔方**自己预判的三处**, 判定「预判 ① 只有 1/3 对」:
「折叠后行首编号是否保留」确实不可断言 (诚实标注正确), 但**由此推出整条 SC 无法断言, 是把
非承重量的不可测当成了承重量的不可测** —— 顺序有与折叠形态无关的测量点 (按出现位置断言内容锚次序)。

⚠️ **而这正是执笔方本轮在另一处 (SC-A-step (c-含)) 亲自走通的"第三条路"** ——
**同一份文件、同一轮里, 一处走通、一处没走**。⇒ 「换测什么而不是放弃测量」这个手法**难在识别时机, 不难在执行**。

### §16.4 位置

A 侧 `max_rounds` **4/4 耗尽**, `converged: false` ⇒ 按 audit-engine §降级策略
触发 **`AskUserQuestion` 三路径**。frontmatter 保持 `degraded: false` (那是路径 [3] 被选中后的结果)。
A 侧 Phase B 仍被阻断 (9 条)。**两条轨现在处于同一结构位置。**

### §16.5 编排层错误累计 **15 条**, 三族

(1) 用一个在该维度上恒真的检查去证实结论 ×3;
(2) **用 AI 的判断/叙述/处置替代一条不该我做或不该我降级的决定 ×3** (豁免闸门 / 只叙述不执行
    `AskUserQuestion` / **漏执行 DEC §5.3 并降级为备忘**);
(3) 只引对我的结论有利的那一段 / 前提没查就用 ×4。
