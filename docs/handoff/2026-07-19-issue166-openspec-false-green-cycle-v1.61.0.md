...
- ❓ **开放问题 (owner 定, 未擅动)**: 规则 #10 目前**全文内联在 Aria 自己的 CLAUDE.md**, 而规则 #7/#8/#9 都在 `standards/conventions/` 有可移植的规范文件。Aria 方法论面向其他项目复用 (Kairos/Aether/SilkNode), 只活在 Aria CLAUDE.md 的规则**不会传播到采用方**。需判定: #10 是 **Aria 本地治理规则** (保持内联即可) 还是**方法论级 convention** (应下沉 standards)? 后者才需要补规范文件。
